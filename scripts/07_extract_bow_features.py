...
def get_wordnet_synonyms(word):
    """
    Fetch synonyms for a word from NLTK's WordNet.
    To avoid semantic drift and clutter, we only query the primary (first) synset,
    and filter out multi-word phrases, acronyms, and proper nouns.
    """
    # Only expand single words (no spaces/hyphens/special chars)
    # Avoid acronyms (all uppercase)
    if " " in word or "-" in word or "&" in word:
        return set()
    if word.isupper():
        return set()
    
    # Also skip short words or words containing numbers
    if len(word) <= 2 or any(c.isdigit() for c in word):
        return set()
        
    # Technical/regulatory exclusion lists (don't expand these)
    no_expansion = {
        "gpu", "gpus", "tpu", "tpus", "cuda", "gdpr", "ccpa", "rag", "dpo", "rlhf",
        "sec", "ftc", "doj", "ai", "ml", "llm", "llms", "rtx", "dgx", "aws", "t5",
        "fiduciary", "patent", "patents", "copyright", "copyrights", "layoff", "layoffs",
        "h100", "a100", "blackwell", "mi300"
    }
    if word.lower() in no_expansion:
        return set()
    ...
    try:
        ...
        # Fallback to empty if WordNet has issues
        return set()
...
# Programmatically expand categories using synonyms and inflections
EXPANDED_WORD_LISTS = {}
for category, word_list in ALL_WORD_LISTS.items():
    expanded_set = set()
    for word in word_list:
        # Get inflections of the parent word
        expanded_set.update(get_inflections(word))
        # Get synonyms and their inflections
        syns = get_wordnet_synonyms(word)
        for syn in syns:
            expanded_set.update(get_inflections(syn))
    EXPANDED_WORD_LISTS[category] = list(expanded_set)

# Print some stats for logging/validation
print("Word list expansion complete:")
for category, original in ALL_WORD_LISTS.items():
    expanded = EXPANDED_WORD_LISTS[category]
    print(f"  Category '{category}': {len(original)} -> {len(expanded)} words")
...
# Group words by their normalized column name to handle collisions and precompile patterns
# We fold all synonyms and inflections back to the parent word's column
words_by_col = {}
for category, word_list in ALL_WORD_LISTS.items():
    for word in word_list:
        col_name = normalize_word_to_feature(word)
        if col_name not in words_by_col:
            words_by_col[col_name] = set()
        
        # Add parent inflections
        words_by_col[col_name].update(get_inflections(word))
        
        # Add synonyms and their inflections
        syns = get_wordnet_synonyms(word)
        for syn in syns:
            words_by_col[col_name].update(get_inflections(syn))

WORD_PATTERNS = {}
for col_name, words in words_by_col.items():
    sorted_words = sorted(list(words), key=len, reverse=True)
    escaped_patterns = [re.escape(w) for w in sorted_words]
