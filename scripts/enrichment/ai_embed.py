...
# --source-relation -> tabla de capas. Un box de GPU sólo necesita tener
# sincronizada la que use (por defecto bronze.unique_paragraphs).
        ...
        try:
            ...
            row = pl.scan_parquet(part).select(
                ...
                *[(pl.col(c).cast(pl.String).drop_nulls().first() if c in columns else pl.lit(None, pl.String))
                  for c in ("model", "dtype")]).collect().row(0)
...
EMPTY_KEYS = pl.DataFrame(schema={"text_hash": pl.UInt64})


def _distinct_hashes(frames: list[pl.LazyFrame]) -> pl.DataFrame:
    if not frames:
        return EMPTY_KEYS
    return (pl.concat([f.select(pl.col("text_hash").cast(pl.UInt64)) for f in frames])
            .unique().collect())


def embedded_keys(usable: list[Path]) -> pl.DataFrame:
    """Already-embedded TEXTS (distinct `text_hash`), keyed by `text_hash` --
    not by PARAGRAPH_KEY. (2026-09-05) The original per-instance design re-embedded
    ...
    return _distinct_hashes([pl.scan_parquet(part) for part in usable])


def embedded_keys_combined(index_path: Path | None, extra_parts: list[Path]) -> pl.DataFrame:
    """Same contract as `embedded_keys`, but unions TWO sources instead of
    picking one: the lightweight index (whatever coverage it already claims)
    plus any local part files NOT already reflected in it (`extra_parts` --
    e.g. a part this box just wrote itself, on top of an index it only
    pulled). Never re-reads a part the index already accounts for. Safe with
    either input empty/None."""
    frames = [pl.scan_parquet(index_path)] if index_path is not None else []
    frames += [pl.scan_parquet(part) for part in extra_parts]
    return _distinct_hashes(frames)
...
def load_embedded_index(output_dir: Path, model_name: str, dtype: str) -> Path | None:
    ...
    `embedded_keys` whenever no local parts are present but this
    index is.
...
def embedded_keys_from_index(index_path: Path) -> pl.DataFrame:
    """Same contract as `embedded_keys`, sourced from the lightweight index
    instead of the full embedding parts."""
    return _distinct_hashes([pl.scan_parquet(index_path)])
...
def write_embedded_index(output_dir: Path, model_name: str, dtype: str, text_hashes,
    ...
    whatever `embedded_keys` set it started the run with -- however that
...
def pending_frame(embedded: pl.DataFrame, limit: int = 0, source: str = "unique_paragraphs",
                  countries: tuple[str, ...] | None = None) -> pl.LazyFrame:
    ...
    embedded_keys' docstring). Either way the output schema is
    ...
    this run."""
    frame = L.scan(SOURCE_TABLES[source])
    if countries:
        frame = frame.filter(pl.col("country_code").is_in(list(countries)))
    frame = (frame.join(embedded.lazy(), on="text_hash", how="anti")
             .select(*PARAGRAPH_KEY, "paragraph_text")
             .sort("form", "country_code", "accession_number", "item_key", "paragraph_index"))
    return frame.head(int(limit)) if limit and limit > 0 else frame
...
def run_embed(
    ...
    if countries:
        source = source.filter(pl.col("country_code").is_in(list(countries)))
    row_count = source.select(pl.len()).collect().item()
    if row_count == 0:
        raise ValueError(f"{source_relation} is empty (for countries={countries}); "
                         f"build it with `make bronze` first")
    embedded_frame = embedded_keys_combined(prior_index, extra_usable)
    embedded = embedded_frame.height
    prior_part_files = sorted(indexed_files)
    coverage = (source.select("country_code", "form",
                              pl.col("text_hash").is_in(embedded_frame["text_hash"].implode()).alias("done"))
                .group_by("country_code", "form")
                .agg(pl.len().alias("paragraphs"), pl.col("done").sum().alias("embedded"))
                .sort("country_code", "form").collect().to_dicts())
    ...
    if verify_only or target_rows == 0:
        ...
        if extra_usable:
            ...
            # `embedded_frame` is already the combined (index +
            # extra_usable) set built above; no rescan needed.
            index_path, n_hashes = write_embedded_index(
                output_dir, model_name, dtype, embedded_frame["text_hash"].to_list(),
    ...
    # Autotune probe: `probe_rows` texts at seeded-random row positions.
    probe_n = max(1, min(probe_rows, row_count))
    positions = np.sort(np.random.default_rng(sample_seed).choice(row_count, size=probe_n, replace=False))
    sample = (source.select("paragraph_text").with_row_index("_row")
              .filter(pl.col("_row").is_in(pl.Series(positions, dtype=pl.UInt32).implode()))
              .collect()["paragraph_text"].to_list())
    plan = autotune_batching(model, [t or "" for t in sample], device, memory_fraction)
    ...
    # (however `embedded_frame` was sourced -- full parts or the
    ...
    prior_hashes = embedded_frame["text_hash"].to_numpy()

    result = pending_frame(embedded_frame, limit, source=source_relation, countries=countries).collect()
    del embedded_frame
    ...
    # Read in chunks of whole 2,048-row vectors, `fetch_rows` rows at most.
    chunk_rows = max(1, fetch_rows // 2048) * 2048
    for offset in range(0, result.height, chunk_rows):
        chunk = result.slice(offset, chunk_rows).to_pandas()
