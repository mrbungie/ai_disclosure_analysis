...
Uso:
    uv run python scripts/enrichment/ai_prefilter_sentences.py
    uv run python scripts/enrichment/ai_prefilter_sentences.py --limit 2000
"""
...
from ai_prefilter_classify import lower_text  # noqa: E402
...
SCORES_DIR = L.INTERIM / "prefilter_scores_unique"
...
def term_regex(terms) -> str:
    """Mismo criterio de límite de palabra que el prefiltro: límite ASCII
    `(?-u:\\b)` sobre minúsculas, con los espacios y puntos escapados."""
    alternation = "|".join(t.replace(".", "\\.").replace(" ", "[ ]") for t in terms)
    return f"(?-u:\\b)({alternation})(?-u:\\b)"
...
def already_covered_hashes(output_dir: Path, model: str, fingerprint: str) -> set[int]:
    ...
    if not parts:
        return set()
    hashes = (pl.concat([pl.scan_parquet(p) for p in parts], how="diagonal_relaxed")
              .filter((pl.col("model") == model) & (pl.col("anchors_fingerprint") == fingerprint))
              .select("text_hash").unique().collect())
    return set(hashes["text_hash"].to_list())


def load_sentences(limit: int = 0, exclude_hashes: set[int] | None = None) -> pd.DataFrame:
    ...
    exclude = pl.DataFrame({"text_hash": sorted(exclude_hashes or [])}, schema={"text_hash": pl.UInt64})
    # Última corrida de scores por texto (de cualquier configuración): sólo se
    # usan los booleanos léxicos, que no dependen de los anchors.
    latest_scores = (pl.concat([pl.scan_parquet(p) for p in sorted(SCORES_DIR.glob("*.parquet"))],
                               how="diagonal_relaxed")
                     .select("text_hash", pl.col("run_id").cast(pl.String), "strong_lexical_match",
                             "weak_lexical_match")
                     .sort("run_id", descending=True, maintain_order=True)
                     .unique("text_hash", keep="first", maintain_order=True))
    candidates = (L.scan("bronze.unique_paragraphs")
                  .select(*keys, "text_hash", "is_scorable")
                  .join(latest_scores, on="text_hash")
                  .filter(pl.col("is_scorable")
                          & (pl.col("strong_lexical_match") | pl.col("weak_lexical_match")))
                  .join(exclude.lazy(), on="text_hash", how="anti")
                  .select(*keys, "text_hash")
                  .collect())
    sentences = (L.scan("bronze.sentences")
                 .filter(pl.col("accession_number").is_in(candidates["accession_number"].unique().implode()))
                 .select(*keys, "sentence_index", "sentence_text"))
    frame = (candidates.lazy().join(sentences, on=keys)
             .filter(lower_text(pl.col("sentence_text")).str.contains(pattern)
                     & (pl.col("sentence_text").str.len_chars() > 15))
             .select("text_hash", "sentence_index", "sentence_text")
             .sort("text_hash", "sentence_index"))
    if limit:
...
def main() -> None:
    ...
    sentences = load_sentences(args.limit, exclude_hashes=covered)
