...
MODO DE ANÁLISIS FINAL: margen extensivo. El panel es el spine
`data/gold/spines/firm_year/firm_year.parquet` (TODAS las empresas-año que
tienen filings) unido con covariates/firm_year/{disclosure_volume,
financials, market} y targets/firm_year/{market, financials}, y las
...
esos documentos sale de una función con nombre, y se persiste en
`data/results/appendix/crosscheck_stats.json` (override con `--json`) para
diffear entre corridas.
...
Cubre:
  ...
  §2 FDR                  las 9 correlaciones del cruce contable/mercado con
                          Benjamini-Hochberg, más el test de permutación
  §3 perfiles             ratios, valuación, beta/volatilidad y retorno por
...
Uso:
    uv run python scripts/analytics/appendix/report_crosscheck_stats.py
    uv run python scripts/analytics/appendix/report_crosscheck_stats.py --json out.json
"""
...
PANEL_FAMILIES = [
    ...
    ("covariates", "financials", ["gross_margin", "operating_margin", "net_margin", "roa", "roe", "current_ratio",
                                  "debt_to_equity", "asset_turnover", "capex_intensity", "rd_intensity", "sic2",
                                  "has_10k"]),
    ("covariates", "market", ["market_cap", "pe_ratio", "pb_ratio", "ev_revenue", "ev_ebitda", "momentum_12_1",
                              "beta", "ps_ratio", "idio_vol_252d", "vol_pre_60d"]),
    ("targets", "market", ["vol_post_60d", "ret_m1_p5"]),
    ("targets", "financials", ["next_revenue_yoy", "next_rd_expense_yoy", "next_capex_yoy", "next_sga_expense_yoy"]),
]
SEED = 42
GROWTH_CLIP = 3.0          # |crecimiento YoY| > 300% se descarta como outlier
PERMUTATIONS = 2000

# Los 9 pares que apendice/correlaciones_fdr_permutacion.md somete a FDR: comportamiento declarado en t contra
...
PROFILE_COLUMNS = ["gross_margin", "operating_margin", "net_margin", "roa", "roe", "rd_intensity",
                   ...
                   "market_cap", "ret_m1_p5", "next_revenue_yoy"]
...
# Firm-year join on the calendar year: the text covers every filing of the year and the outcomes are
# anchored to the year's 10-K, so these are associations, not predictions.
TIMING = ("descriptive, contemporaneous: calendar-year AI text against firm-year outcomes anchored to the 10-K "
          "(t+1 revenue growth included); not point in time")


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--json", type=Path, default=L.results_path("appendix", "crosscheck_stats.json"))
    args = parser.parse_args()

    master = L.read_dataset("firm_year", *PANEL_FAMILIES)
    roic = L.read_gold("firm_year", ("covariates", "financials"))
    # Unreported long-term debt read as zero debt (gold leaves ROIC/WACC null there).
    roic[["roic", "wacc", "roic_minus_wacc"]] = roic_wacc_debt_as_zero(roic)
    master["nivel_ia"] = intensity_level(master)
    report: dict = {"timing": TIMING, "n_firm_years": int(len(master)), "n_firms": int(master["ticker"].nunique()),
                    "share_any_ai": float(master["any_ai"].mean()),
                    # todas las empresas-año del spine tienen texto: el panel condicional es el panel
                    "n_in_conditional_panel": int(len(master))}
    ...
    print("\n" + "=" * 78); print("§2  LAS 9 CORRELACIONES DEL CRUCE, EN INTENSIDAD, CON FDR (Benjamini-Hochberg 5%)"); print("=" * 78)
    results = []
    ...
    for x in ("any_ai", "frames_per_1k"):
        for y in ("next_revenue_yoy", "next_capex_yoy", "next_rd_expense_yoy", "ret_m1_p5"):
