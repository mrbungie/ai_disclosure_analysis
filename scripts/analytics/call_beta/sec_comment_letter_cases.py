...
Deterministic, no LLM. Reads silver.firm_universe and the gold tables
spines/document/document + covariates/document/disclosure_volume (10-K frame
counts), covariates/firm_year/washing_score (w) and covariates/firm_year/market
(market_cap). Writes data/results/call_beta/sec_comment_letter_cases.json.
"""
...
OUT = REPO_ROOT / "data" / "results" / "call_beta" / "sec_comment_letter_cases.json"
N_CONTROLS = 3
KS = [-2, -1, 0, 1]
...
def load_10k_panel() -> pd.DataFrame:
    universe = (L.scan("silver.firm_universe").filter(pl.col("country_code") == "us")
                .select("ticker", "sic").collect().to_pandas())
    universe["sic2"] = universe["sic"].astype(str).str.zfill(4).str[:2]
    docs = L.read_gold("document", ("covariates", "disclosure_volume")).drop(columns="id")
    ...
    firm_year = L.read_dataset("firm_year", ("covariates", "washing_score", ["w"]), ("covariates", "market", ["market_cap"]),
                            spine_columns=["id", "ticker", "year"])
    docs = docs.merge(firm_year.drop(columns="id"), on=["ticker", "year"], how="left")
