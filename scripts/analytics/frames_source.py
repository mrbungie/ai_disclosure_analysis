...
def _base_sql(manifest: str, form_expression: str) -> str:
    return f"""
        SELECT fm.ticker, fm.cik, {form_expression} AS form_type, fm.filing_date,
               ...
               f.text_hash, f.frame_index, f.subject, f.ai_type, f.temporal, f.domain,
               f.concepts, f.rhetoric_promotional, f.rhetoric_strategic_importance,
...
def load_frames_parquet(include_10q: bool = False,
    ...
    try:
        con.execute(f"""
            CREATE TEMP VIEW all_frames AS
            ...
            CREATE TEMP VIEW latest_call AS
            ...
            CREATE TEMP VIEW current_population AS
