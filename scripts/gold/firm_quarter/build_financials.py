"""
scripts/gold/firm_quarter/build_financials.py — firm-quarter financials from the
10-Q/10-K XBRL facts, combining two US XBRL sources:

1. Inline-XBRL per-filing facts (`bronze.xbrl_facts`, built from
   data/raw/xbrl_facts/us_by_filing/ by scripts/bronze/xbrl_facts.py from
   scripts/raw_processing/us/04_extract_inline_xbrl_facts.py) — primary
   source, one value per (ticker, calendar quarter), EARLIEST-filed value
   wins (as-of safe: see `first_disclosed()`).
...
`quarterly_panel()`: one row per (ticker, calendar quarter of the fiscal
period end, metric), with `source` (inline_xbrl | frames_fallback),
`source_ref` (accession number) and `filing_date` (of the filing the value
was first inferable from), so provenance is always traceable back to a
specific filing.

Output: covariates/firm_quarter/financials, point in time on the
firm-quarter spine: for every metric, `<metric>` is the value of the latest
fiscal quarter whose filing_date is before as_of_date, and
`<metric>_quarter` the calendar quarter of that fiscal period's end. Null
before the firm's first filed value.
"""
...
def quarterly_panel() -> pd.DataFrame:
    ...
    base = (L.scan("bronze.xbrl_facts")
             .filter(pl.col("concept").is_in(ALL_TAGS))
             .select(["ticker", "accession_number", "filing_date", "has_dimensions",
    ...
    manifest = pl.concat([L.scan("silver.filing_manifest").select("accession_number", "filing_date"),
                          L.scan("silver.filing_manifest_10q").select("accession_number", "filing_date")]
                         ).collect().to_pandas()
    frames = pd.read_parquet(REPO_ROOT / config["storage"]["raw_xbrl_frames_alt"] / "us_10q_frames.parquet")
    ...
    bank_ref = (bank_parts.sort_values("filing_date")
                          ...
                          .set_index(["ticker", "cal_q"])[["accession_number", "filing_date"]])
    ...
    for metric, tags in TAGS.items():
        ...
        sub = sub[["ticker", "cal_q", "numeric_value", "accession_number", "filing_date"]].rename(
        ...
        if metric == "revenue" and not bank_revenue.empty:
            ...
            sub["filing_date"] = sub["filing_date_bank"].where(has_bank, sub["filing_date"])
            sub["source"] = sub["source"].fillna("inline_xbrl")
            sub = sub.drop(columns=["value_bank", "accession_number", "filing_date_bank"])

        fsub = frames[frames["metric"] == metric][["ticker", "cal_q", "value", "accession_number", "filing_date"]].rename(
...
def main() -> None:
    ...
    out = spine.copy()
    for metric, rows in panel.groupby("metric", sort=False):
        rows = rows.sort_values(["ticker", "filing_date", "cal_q"])
        # a filing extends the known history only when it discloses a later
        # fiscal quarter than every earlier filing of the firm
        rows = rows[rows["cal_q"] == rows.groupby("ticker")["cal_q"].cummax()]
        rows = rows.drop_duplicates(["ticker", "cal_q"], keep="first")
        known = pit.asof_join(spine[["ticker", "as_of_date"]],
                              rows[["ticker", "filing_date", "value", "year", "quarter"]],
                              event_date="as_of_date", snapshot_date="filing_date", strict=True)
        out[metric] = known["value"].values
