...
Determinístico, sin LLM. Requiere las capas silver/bronze y las familias gold
firm_year que lee `incremental_signal.py::load()`.

Salida: `data/results/washing/firm_year_aggregation_robustness.json`
"""
...
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "shock"))
...
def build_altagg_panel() -> pd.DataFrame:
...
def main() -> None:
    m = isig.load()             # panel principal (mediana), sin tocar
    alt = build_altagg_panel()
    ...
    out_path = L.results_path("washing", "firm_year_aggregation_robustness.json")
