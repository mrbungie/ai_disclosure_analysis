"""
scripts/us/10k/01_fetch_filings.py — builds the 10-K manifest AND fetches
...
10-K only, into its own storage (data/raw/filings_html/,
data/interim/manifests/filing_manifest.parquet) — the panel core (never
pooled with the 10-Q shock series — see the project's data-scope
decision).

Usage:
    uv run python scripts/us/10k/01_fetch_filings.py
"""
...
def main():
    ...
    edgar_fetch.configure(
        ...
        local_storage_dir=Path(".edgartools_data") / "10k",
    )

    filing_date = config["corpus"]["filings"]["filing_date"]
    edgar_fetch.fetch_filings(
        universe_df=universe_df,
        form="10-K",
        start_date=filing_date["from"],
        end_date=filing_date["to"],
        allow_amendments=config["corpus"]["filings"].get("amendments", False),
        html_dir=Path("data/raw/filings_html"),
