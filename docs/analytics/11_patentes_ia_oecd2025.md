...
* **Scripts de Pipeline**:
  * `scripts/analytics/build_sp500_patent_aliases.py`: Extracción y expansión de nombres, más el diccionario curado de subsidiarias/nombres históricos.
  * `scripts/analytics/upload_aliases_to_bigquery.py`: Carga hacia BigQuery.
  * `scripts/analytics/build_oecd_patents_panel.py`: Generador de SQL con las 100 clases CPC y 274 keywords.
  * `scripts/analytics/run_oecd_ai_patents.py`: Ejecutor integral que procesa los ~170M de registros de patentes (filas patente × assignee) y persiste el panel procesado.
