...
`attach_asof()` en `scripts/analytics/build_firm_financials.py`.
...
`scripts/analytics/call_car_regressions.py`. La misma lógica es la que
...
Ejemplo real: `risk_free_at()` en `scripts/analytics/build_roic_wacc.py`.
...
Ejemplo real: `attach_asof()` en `scripts/analytics/build_firm_financials.py`.
...
`scripts/us/docs/sp500_2021_start_panel_correction.md`: el universo crudo
