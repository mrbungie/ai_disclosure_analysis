...
## US — `scripts/raw_ingestion/us/03_fetch_accounting_data.py`
...
`scripts/raw_ingestion/us/edgar_fetch.py` for the narrative filings) as
...
`scripts/raw_processing/us/04_extract_inline_xbrl_facts.py` exists to avoid for the 10-Q
shock series (see below). `scripts/gold/financials/build_firm_financials.py`
...
## Chile — `scripts/raw_ingestion/cl/04_fetch_accounting_data.py`
...
`scripts/raw_ingestion/cl/04_fetch_accounting_data.py` reuses `01_fetch_filings.py`'s
...
`scripts/raw_processing/us/04_extract_inline_xbrl_facts.py` parses inline XBRL directly out
of each cached 10-Q filing's own HTML (point-in-time, firm's own fiscal
quarter). `scripts/raw_ingestion/us/05_fetch_xbrl_frames_alt.py` pulls the same 13 core
...
in `scripts/gold/financials/build_us_10q_financials_panel.py` recovers them by
...
`scripts/gold/financials/build_us_10q_financials_panel.py` into
`data/processed/us_10q_financials_panel.parquet`.

## Actual run results (2026-09-03)

**US** (`scripts/raw_ingestion/us/03_fetch_accounting_data.py`, 516 tickers): 514
...
**Chile** (`scripts/raw_ingestion/cl/04_fetch_accounting_data.py`, 100 firms ×
