...
`scripts/bronze/ai_prefilter_classify.py` es el paso que faltaba: §8.1 solo
...
`scripts/bronze/ai_classify.py` (extracción de frames semánticos) debe
...
`scripts/bronze/ai_prefilter_classify.py`) que fuercen `is_ai_prefiltered
...
`scripts/bronze/ai_classify.py` (qwen/qwen3.7-flash por OpenRouter) —
...
parquet`, de `scripts/raw_processing/cl/cmf_pdf_paragraphs.py`) que nunca entraban a
...
- **`scripts/bronze/ai_entity_mentions.py`**: sobre la población
...
Memoria chilena — ver `scripts/raw_processing/us/8k/segmenter_8k.py`) sobre 34.421
...
del párrafo venía promediando. `scripts/bronze/ai_prefilter_sentences.py` embebe
...
**Desplegado** (`run=20260906T005225Z`, `scripts/bronze/ai_prefilter_deploy.py`):
