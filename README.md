...
Current results (`reports/validation_summary__rule_based.txt`, n=231 after dedup — 236 labeled minus 5 chunk_ids orphaned by the aip/aiops re-chunk, see below):

| Dimension | F1 | Precision | Recall | Accuracy |
|---|---|---|---|---|
| is_ai_related | 0.900 | 0.818 | 1.000 | 0.818 |
| is_substantive | 0.708 | 0.874 | 0.595 | 0.654 |
| is_promotional | 0.357 | 0.235 | 0.741 | 0.688 |
| is_risk_related | 0.749 | 0.761 | 0.737 | 0.796 |
| is_governance_related | 0.634 | 0.571 | 0.711 | 0.840 |

Specificity score vs. LLM-judged specificity: Spearman ρ = 0.293 (p < 0.001). These figures quantify where the deterministic pipeline proxies are reliable (AI relevance, risk, substantiveness) and where they are noisier (promotional tone, governance) — a required input for treating downstream washing claims as findings rather than assertions. **These are still preliminary / in-sample-tuned numbers** (see the boolean-formula caveat below) — pending a CV search-pool/holdout rework, tracked in `docs/methodology_iteration.md`.
...
**Promotional/governance proxy fix (script 09, `build_proxy_sql`).** The original `is_promotional_proxy` fired on named-product mentions and deployment verbs ("launched", "copilot", "subscription") — i.e. "mentions a shipped AI product," not promotional *tone* — giving F1=0.277 (P=0.182, R=0.581) against the LLM judge on the validation sample. It's now `count_vague_words > count_negative_words AND NOT has_risk_factor AND bow_sentiment_score >= 0` (hype vocabulary outweighing negative-tone words, outside risk-factor language, non-negative overall sentiment), which measures F1=0.357 on the same sample. `is_governance_related_proxy` gained a `has_compliance AND has_word_ai` clause (previously only board-oversight/audit-committee/ethics-policy), moving recall from 0.333 → 0.711 and F1 from 0.500 → 0.634. Both formulas were selected by grid-searching candidate boolean combinations of existing BoW features against this same validation set — an in-sample tuning caveat: report these numbers as "fit to this sample," not as an independent generalization estimate. A proper CV search-pool/holdout split for these 4 dimensions is built (`val_03_dedup_split.py`) but not yet run through a search harness or a locked holdout — see `docs/methodology_iteration.md`.

**Prefilter/chunking recall check (scripts 05–06; `val_08`–`val_11`).** The precision numbers above only cover chunks that already made it into `ai_candidate_chunks.parquet` — they say nothing about AI content the keyword prefilter never surfaced. Two failure modes were checked directly: 142/658 filings (21.6%) never match the keyword regex at all and are never chunked; and within matched filings, ~618K/634K paragraphs fall outside the +/-1 keyword-hit window and are never chunked either. An industry-stratified LLM-judge sample (30/30 and 55/55 industries covered) found one confirmed miss — Palantir's "AIP" product name, not caught because `\bai\b` doesn't match inside "AIP" — which was fixed by adding `aip`/`aiops` as literal keywords (verified against the full corpus first: `aip` only ever appears for PLTR, `aiops` only for PANW/DDOG/AVGO, zero false-positive risk, unlike a broader `AI[A-Z]+` pattern which mostly matched noise like AIDS/AICPA/aircraft). The pipeline was re-run end to end after the fix (4,204 → 4,228 chunks). A **fresh, single-look holdout** (534 paragraphs, never used to find or tune the fix) then estimated the *post-fix* recall at **≈0.83** (`reports/prefilter_recall_holdout_validation.txt`) — TP and FN both counted in raw paragraphs, not chunks, since each chunk merges ~3.5 paragraphs on average and mixing the two units understates recall. That holdout also surfaced a second gap (Snowflake using "ML" as a standalone acronym) — documented here but deliberately left **unfixed**: acting on it would burn this holdout the same way the search-pool sample was burned by the aip/aiops discovery. Fixing it requires sampling and judging a third, never-seen batch first.
