...
def caps_headings(document: Document) -> None:
    """Draw every Heading 1 in full capitals.

    The runs have to say it outright: the style cannot be relied on, since
    cover.docx is the master of the docxcompose merge and the merged file keeps
    ITS Heading 1 definition. Capitals need no recasing of the text, so the
    heading stays in ordinary sentence case in the source, which is how a
    heading should read there, and inline formatting is untouched.
    """
    heading_ids = set()
...
def upgrade_compatibility_mode(document: Document) -> None:
    """Take the merged file out of Word 2003 compatibility mode.

    cover.docx is the master of the docxcompose merge and it descends from a
    Word 97-2003 `.dot` template, so its settings.xml asks for
    `compatibilityMode` 11. Native OMML equations do not exist in that mode:
    Word renders them on open, then converts every one of them to a VML
    picture the moment the file is saved, and the equations stop being
    editable or searchable text. Declaring the modern mode is what keeps them
    as equations through a round trip in Word.
    """
    ...
    for setting in compat.findall(qn("w:compatSetting")):
        if setting.get(qn("w:name")) == "compatibilityMode" and setting.get(qn("w:uri")) == uri:
            setting.set(qn("w:val"), WORD_COMPAT_MODE)
            return
    compat.append(compat.makeelement(qn("w:compatSetting"), {
        qn("w:name"): "compatibilityMode", qn("w:uri"): uri, qn("w:val"): WORD_COMPAT_MODE,
    }))


def mark_fields_dirty_and_save(document: Document, out_path: Path) -> None:
    fix_hyperlink_style(document)
    upgrade_compatibility_mode(document)
