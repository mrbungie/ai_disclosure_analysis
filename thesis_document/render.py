...
def titlecase_headings(document: Document) -> None:
    ...
    Words that already carry a capital anywhere (AI, SIC, XBRL, 10-K) are left
    exactly as written. Title-casing never changes a string's length, so the
    result can be redistributed over the paragraph's runs character by
    character, keeping any inline formatting (italics, math, links) intact
    instead of collapsing the heading into a single run.
    """
    heading_ids = set()
    ...
    for paragraph in document.paragraphs:
        ...
        cased = _WORD_RE.sub(
            lambda m: m.group(0) if m.group(0) != m.group(0).lower() else m.group(0)[0].upper() + m.group(0)[1:],
            text,
        )
        if cased == text:
            continue
