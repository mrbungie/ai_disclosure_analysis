...
The texts labelled to train and evaluate the models sit outside the inference funnel and are listed in @tbl-labelled. The prefilter is trained on an LLM-judged golden set of 9,900 unique 10-K and 10-Q texts with `GroupKFold` grouped by filing, so paragraphs from the same document never span train and test, tuned on 2,393 further proxy-statement and 8-K texts, and evaluated on a 1,500-text proxy and 8-K holdout that never enters any fit or threshold choice. The judge itself is checked against an independent model (Gemini) on 6,038 paired texts, with κ = 0.87 on whether a paragraph mentions AI and 0.86 on whether it is AI disclosure [@cohen1960]. Performance differs by venue, and the difference is carried as a caveat through the whole dissertation: precision is 0.98 in 10-K and 10-Q text and 0.73 in proxy statements and 8-Ks; the weighted F1 is 0.96 in 10-K text against 0.82 in DEF 14A and 0.65 in 8-K. The prefilter has not been validated on earnings-call transcripts, the venue with the most frames per paragraph; the pending human sample is stratified by venue for that reason. Every comparison *between* venues therefore mixes a difference in discourse with a difference in measurement error; comparisons *within* a firm over time do not.
...
| Earnings calls | calls | **0** | — | — | the prefilter is not validated on calls | none |
...
Both venues pass through the same prefilter and judge with different measurement error (precision 0.98 in 10-K and 10-Q, 0.73 in proxy and 8-K, and no validation on calls); part of the *level* of the gap may be differential error, but the within-firm event study does not suffer from it. Intensity per paragraph depends on genre: a call has about 70 paragraphs and a 10-K about 500, so the gap in frames per 1,000 paragraphs is also a gap in genre density, which is why the share-based check is reported. Transcripts cover about 3.2 of 4 calls per firm-year and end in mid-2025. The promotional flag is an LLM label pending human validation. Designs the gap does not support are stated rather than attempted: a difference-in-differences with treatment defined by the prior gap is mean reversion; a synthetic control needs a longer pre-period than four annual points.
