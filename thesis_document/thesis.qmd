...
A sequential variance-decomposition exercise makes this asymmetry between disclosure volume and substance explicit for valuation specifically: adding semantic posture and disclosed AI activity to a model that already includes raw AI mention counts raises explanatory power for price-to-sales by $+4.2$ percentage points, while mention counts alone add essentially none (@fig-nlp-horse-race). The same decomposition shows the reverse pattern for market-risk outcomes -- beta, idiosyncratic volatility, and next-year revenue growth -- where raw mention counts already capture most of the explanatory power and posture and activity add comparatively little on top. Once posture and disclosed activity are held constant, the decoupling index ($W$) contributes virtually no additional explanatory power anywhere ($\Delta R^2 \le 0.0009$), consistent with the null association already established for decoupling and crash risk. Distinguishing volume, posture, and disclosed substance therefore does reveal real economic differences in what markets price -- just not the differences $H_{6b}$ anticipated: valuation rewards *how* AI is described, market risk rewards *how much*, and neither rewards the verifiable gap between the two.
...
A sequential horse race evaluates the incremental explanatory power of each NLP layer over financial fundamentals (Model 0: sector-by-year fixed effects, market capitalization, profit margins, and asset turnover) by adding, in order: raw AI mention counts (Model 1: total AI disclosure intensity, $\log(1 + \text{AI frames})$ or mentions per 1,000 words), semantic frame posture (Model 2: the full multi-dimensional communication posture -- risk, governance, temporal, positioning, and specificity), disclosed AI activity inventory (Model 3: concrete operational claims and grounded behavioral features, such as named business functions, deployed stages, and product or process citations), and the decoupling/washing index $W$ (Model 4: the within-disclosure rank gap between talk and disclosed substance). @fig-nlp-horse-race, presented in the main text, reports the resulting $\Delta R^2$ across all five firm-level outcomes; @tbl-c1-m0m3 reports the full M0-M3 model-fit statistics underlying that decomposition for beta.
