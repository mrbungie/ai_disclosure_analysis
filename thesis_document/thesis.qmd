...
def gy(family, year):
    return f'{_gap_year[family][str(year)]:+.1f}'
```

- **Quantified outcomes:** the call premium on numbers is already there before ChatGPT and stays there (`{python} gy("quantified_outcome", 2021)` pp in 2021, `{python} gy("quantified_outcome", 2023)` pp in 2023, `{python} gy("quantified_outcome", 2025)` pp in 2025). Managers have always put more figures into the oral discussion than into the filing.
- **Named products and processes:** this gap instead widens with generative AI. It is small in the early cycles (`{python} gy("named_product_or_process", 2021)` pp in 2021, `{python} gy("named_product_or_process", 2022)` pp in 2022) and much larger by the end of the panel (`{python} gy("named_product_or_process", 2024)` pp in 2024, `{python} gy("named_product_or_process", 2025)` pp in 2025).
- **Governance and restrictions:** the filings absorb almost all of the growth in governance and risk caveats after 2023. The gap is modest at the start (`{python} gy("governance_or_restriction", 2021)` pp in 2021) and widens once the regulatory warnings arrive (`{python} gy("governance_or_restriction", 2024)` pp in 2024, `{python} gy("governance_or_restriction", 2025)` pp in 2025, `{python} gy("governance_or_restriction", 2026)` pp in 2026).

The quantification premium on calls is therefore an old habit, while the split on product-level claims and on governance language opens specifically as generative AI spreads and regulatory attention grows. These are yearly means of the same within-firm-year gap and carry no inference of their own, so they describe the direction of the movement rather than test it.
