...
Following ChatGPT's release in late 2022, artificial intelligence disclosure became nearly universal across the S&P 500, rendering simple keyword counts incapable of distinguishing substantive execution from promotional rhetoric. This thesis develops an open AI Disclosure Credibility Framework that separates narrative volume ($D$), communication posture ($P$), disclosed operational activities ($A$), and documentary grounding ($G$). The framework translates these constructs into a sequential three-step due-diligence workflow: **identify communication posture, examine documentary grounding, and compare disclosure across corporate channels**.

Analyzing `{python} hd_paragraphs_m` million paragraphs from `{python} f"{hd_n_docs:,}"` SEC filings and earnings calls, the study catalogs `{python} f"{hd_n_activities:,}"` operational AI activities across `{python} hd_n_activity_firms` firms. Communication posture predicts documented execution beyond narrative volume: at comparable disclosure intensity, Vocal Substantives report broader activity portfolios and stronger customer-facing integration than Governance-Led or Defensive peers. Most disclosed initiatives reflect applied integrations of external vendor tools rather than proprietary foundation models; crucially, the 2023–2025 narrative surge expanded activity volume without raising average evidentiary grounding.

Decoupling between narrative volume and disclosed substance ($W$) behaves as an enduring firm characteristic, persisting significantly across reporting cycles rather than acting as transient reporting noise. Within the same firm and fiscal year, voluntary earnings calls carry substantially higher promotional intensity and quantified claims than statutory 10-Ks, revealing systematic cross-venue compositional gaps. Regulatory scrutiny, exemplified by the SEC's comment letter to Welltower, disciplines this disparity by prompting formal clarification in subsequent filings.

Capital market regressions show that investors price historical disclosure habits rather than quarter-specific operational substance. Accumulated historical decoupling reliably predicts higher systematic beta, whereas an individual call's decoupling provides no incremental signal across studied outcomes. Concurrently, persistent disclosure volume and governance-oriented framing associate with lower systematic risk and larger firm capitalization.

Rather than classifying firms as fraudulent or technologically sophisticated, the framework provides actionable due diligence: it enables corporate issuers to **benchmark and review their own communications** against documented execution, while helping investors and boards **prioritize which external claims require deeper verification** before allocating capital.
...
Corporate AI disclosure has become increasingly difficult to interpret. Investors and analysts need to distinguish firms with substantive AI adoption from those making primarily promotional claims, because genuine adoption may affect productivity and firm value [@basnet2025; @song2026]. Managers and boards face the same problem from the inside: they must communicate technical initiatives without overstating their maturity, since the gap between what a firm says and what it can show is now a live legal and reputational exposure. Regulators have made that exposure concrete, naming overstated AI capability as a form of misstatement [@gensler2023; @sec2024pressrelease], and SEC comment letters show that inconsistencies between promotional earnings-call statements and formal filings can draw direct scrutiny [@welltower2025letter]. Regulation is therefore the setting in which this problem has teeth; the readers this thesis writes for are the firms making the claims and the investors pricing them.
...
This thesis examines whether corporate AI disclosures contain sufficiently grounded operational evidence to support credible assessments of corporate claims. It develops an open measurement framework that audits corporate AI communication across `{python} f"{hd_n_docs:,}"` SEC filings and earnings-call transcripts, evaluating credibility across texts, disclosure channels, regulatory comment letters, and market prices.

The thesis makes three contributions:

1. **A practical credibility framework for issuers and investors:** provides diagnostic tools for corporate issuers to review their disclosures against documented execution, and for investors to benchmark and screen external claims during due diligence.
2. **Empirical evidence across corporate and market settings:** links narrative disclosure to operational activities, cross-venue credibility gaps between earnings calls and statutory filings, and market pricing of systematic risk.
3. **A multi-dimensional textual measurement architecture:** moves beyond bag-of-words keyword counting by combining gradient-boosted filtering with zero-shot LLM frame extraction into six constructs ($D, P, A, G, S, W$) that reconcile informal earnings calls with statutory filings.

## Theoretical foundations and state of the art

Research in accounting and finance shows that corporate textual narratives convey economic information beyond structured financial data: @li2008 demonstrates that readability predicts earnings persistence, @loughran2011 show that general-purpose dictionaries misclassify financial terminology, and @gentzkow2019 establish formal statistical frameworks for text as high-dimensional economic data. In the AI context, recent studies document a sharp post-2022 narrative surge [@anantharaman2026]. To evaluate whether that surge reflects substance or opportunism, researchers have separated actionable plans from boilerplate [@basnet2025], benchmarked disclosures against external AI hiring and patent data [@song2026; @babina2024; @eisfeldt2023; @barrios2025], or documented how managers use tech-boom buzzwords to inflate valuations without fundamentals [@cheng2019; @huang2014]. Regulators have concurrently targeted "AI washing" as actionable misstatement under securities law [@gensler2023; @sec2024pressrelease; @welltower2025letter], conceptually extending classic models of greenwashing and selective disclosure [@delmas2011; @marquis2016; @seele2022].

Rather than relying solely on external hiring or patent proxies, this study evaluates whether the disclosure itself is internally consistent and factually grounded. Four theoretical mechanisms underpin this measurement:

- **Costly signalling and verification:** Under classical signalling theory [@spence1973; @crawford1982], ungrounded narrative claims constitute costless "cheap talk," whereas disclosures that name specific operational systems, commercial partners, deployment stages, and quantitative metrics incur verification and reputational costs [@dye1985; @rogers2011]. This mechanism motivates our evidentiary grounding construct ($G$).
- **Institutional isomorphism:** Institutional theory [@dimaggio1983; @meyer1977] predicts that firms subject to common regulatory scrutiny and peer pressure adopt homogeneous rhetoric regardless of operational changes. Measuring raw AI mention volume ($D$) captures this mimetic pressure.
- **Organizational decoupling:** Following @westphal1994, @crilly2012, and @bromley2012, organizations frequently decouple formal policy statements from daily operations. The gap between narrative volume and documented operational activities ($W$) operationalizes this decoupling directly.
- **Channel liability differential:** Because statutory filings (Form 10-K) carry strict liability under Section 11 and Section 18 of the Exchange Act, whereas earnings calls offer broader safe harbor protections for forward-looking statements [@frankel1999; @matsumoto2011], comparing identical firms across venues reveals how regulatory discipline constrains promotional rhetoric.
...
The empirical pipeline converts raw corporate disclosures into structured indicators through a multi-stage architecture (@fig-pipeline-architecture). Candidate paragraphs from SEC filings and earnings calls are deduplicated and screened by a supervised GBDT prefilter ($\tau = 0.17$), from which a zero-shot LLM extracts structured claims and concrete operational activities. These outputs form six core constructs ($D, P, A, G, S, W$) fused with SEC XBRL fundamentals and market returns across four analytical designs: disclosure archetypes, cross-channel comparisons, event studies, and risk regressions. Appendix A details the full acquisition, schemas, and aggregation procedures.

*Scope note on SEC scrutiny and DeepSeek.* The observation window covers two external events: the late-January 2025 DeepSeek release and SEC comment letters addressing AI disclosure. The empirical tests estimate whether these events altered disclosure posture, promotional language, or cross-channel divergence.

Chapters 2 through 5 present the empirical findings, Chapter 6 operationalizes them into an operational due-diligence framework, and Chapter 7 concludes.

# Corporate AI Disclosure Strategies

How did corporate artificial intelligence communication evolve as the technology entered the mainstream, and do firms adopt homogeneous or distinct disclosure strategies? This chapter establishes the empirical foundation of this thesis: it defines the fixed S&P 500 study universe, introduces the multi-source disclosure corpus, outlines the measurement architecture transforming narrative text into six analytical constructs, analyzes market-wide diffusion patterns from 2021 through 2026, and identifies recurring managerial communication postures across large-cap enterprises.
...
Consequently, investors should consult both venues rather than relying on either in isolation, while corporate issuers must review the substantive consistency of material claims across oral and statutory disclosures, without expecting identical wording or equal detail across channels.
