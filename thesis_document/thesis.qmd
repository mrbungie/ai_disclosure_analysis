...
Separating business purpose from operational action distinguishes where AI is intended to add value from what the firm is concretely doing to implement it.
...
The gap is not verbosity: at identical narrative volume, Vocal Substantives still report `{python} f"{hd_vs_ratio:.2f}"` times as many operational AI activities as Defensive Disclosers and Governance-Led Disclosers `{python} f"{hd_gl_ratio:.2f}"` times as many (both $p < 0.001$; the negative binomial conditioning on total AI disclosure volume is in Appendix C). Posture thus helps focus the initial review, but because outcome metrics remain scarce across all groups, an analyst must verify the specific factual backing of each individual initiative.
...
Where AI enters the business barely changes from one archetype to the next (@fig-archetype-activity-mix). Selling -- AI inside the products and services customers buy -- dominates all three groups, with Vocal Substantives leaning on it hardest and Governance-Led Disclosers splitting more evenly between selling and internal operations. Even among Governance-Led firms, risk and compliance work forms a minor fraction of reported deployments: a firm can communicate extensive supervisory oversight while directing its actual AI use to commercial and operational tools. The actual portfolio of initiatives cannot simply be deduced from the posture label. Three filings show what that looks like on the page (@tbl-archetype-activity-examples).
