...
What ties the series to the event is the text rather than the timing: analysts name the model, AT&T's call on `{python} _dsj["quotes"][0]["date"]` opening "you've seen DeepSeek has created a much lower cost, large language model" and Corning's two days later on "market dislocation over the last week from DeepSeek". The series is earnings calls alone, since a call can respond within weeks while a 10-K published in a given month was written earlier; the pooled series shows no break, consistent with that dilution without establishing it. January is not entirely after the 20th, so this locates a change around the turn of the year, not on a day. Analysts must therefore distinguish transient market attention to an emerging technology from durable changes in disclosed corporate initiatives.

## What disclosed AI activity adds to disclosure posture

Disclosed activity carries information that posture alone does not: at equal disclosure volume, Vocal Substantives report `{python} f"{hd_vs_ratio:.2f}"` times as many AI activities as Defensive Disclosers, confirming that archetypes do not proxy for verbosity. For corporate issuers, this activity inventory provides an objective basis to benchmark specific technical initiatives and evidentiary backing against peers; for investors, it targets due-diligence questions directly toward deployment maturity, quantified outcomes, and third-party vendor reliance. However, firms retain wide discretion over what they emphasize within any single document, raising the question of whether these operational claims survive as legal and regulatory commitment escalates across communication channels -- the focus of Chapter 4.

# Credibility Under Increasing Disclosure Commitment

How credible are corporate AI claims when scrutinized against escalating legal and regulatory commitments? Because enterprise AI adoption lacks an audited reporting census, this thesis evaluates credibility by observing what survives as the cost of misrepresentation rises [@crawford1982; @rogers2011]. What a firm leaves out is a choice as much as what it includes: @dye1985 shows that when investors cannot tell whether a manager holds information at all, withholding unfavourable detail carries no penalty -- which is why the absence of a named system or a figure is treated here as evidence about the claim rather than as missing data. Credibility is tested through three complementary designs: (1) within-document decoupling between disclosure intensity and disclosed substance; (2) cross-venue comparison of activity composition and grounding from voluntary earnings calls to statutory filings; and (3) disclosure responses to SEC comment-letter scrutiny. Together, these tests move from internally observable substantiation to externally consequential disclosure settings (@tbl-credibility-designs).
...
This construct operationalizes established concepts of organizational decoupling [@meyer1977; @westphal1994; @bromley2012] and symbolic disclosure [@delmas2011; @lyon2015; @elhajjar2025] into an observable comparison within public filings: how much a firm discusses AI relative to the grounded operational activity it puts on the record.
...
Firms that talk about AI at comparable intensity end up far apart on substance (@fig-ds-dumbbell). Across the panel $W$ is widely dispersed ($\text{standard deviation} =$ `{python} f"{std_w:.2f}"`), with `{python} n_tail` firm-years in the decoupled tail at the 95th percentile ($W \ge$ `{python} f"{cutoff:.2f}"`) -- not firms with no AI interest, but ones that talk about AI heavily while disclosing almost no verifiable operational substance. The tail is where an analyst looks first: a firm ranking in the top quintile on disclosure and the bottom quintile on substance is making claims its own filings do not support. For reporting companies, a high decoupling rank serves as an internal diagnostic to review whether concrete initiative details are missing or whether broad claims need pruning, without presuming in advance which is the case. The gap is also an enduring posture rather than a reporting accident: it persists from one year to the next at a year-over-year Spearman $\rho =$ `{python} f"{persistence_rho:+.3f}"` (Appendix D).
...
Consequently, investors should consult both venues rather than relying on either in isolation, while corporate issuers must review the substantive consistency of claims across oral and statutory disclosures, without expecting identical wording or equal detail across channels.
...
In practice, analysts can prioritize corporate review by focusing on persistent decoupling and sharp cross-venue discrepancies. The next question is whether these disclosure differences correspond to observable economic outcomes: Chapter 5 examines whether capital markets and firm fundamentals reflect these narrative and substantive signals.

# Market Relevance of Disclosure Signals

This chapter asks whether the disclosure measures built in this thesis carry information that equity markets price. The unit of observation is an earnings call held by an S&P 500 constituent between 2021 and 2026, evaluating whether predetermined disclosure signals are associated with post-call market and fundamental outcomes. Following @gelman2014, these regressions serve as exploratory diagnostics across three motivating theoretical channels: narrative volume pricing as thematic risk exposure [@eisfeldt2023], disclosed substance operating as a costly signal [@spence1973; @verrecchia1983], and decoupling accumulating as opacity that surfaces in tail risk [@jin2006; @fu2021].

## The disclosure construct and the estimating equation

Every disclosure regressor is predetermined relative to the call via a backward as-of join on the call date, ensuring a call never carries information published after it. @tbl-market-regressors lists the full set of regressors and controls.
...
Because archetype weights sum to one, the Defensive posture is held as the reference vertex; coefficients on $w_{\text{voc}}$ and $w_{\text{gov}}$ capture compositional reallocations of weight away from Defensive, where a 10 percentage-point shift corresponds to $0.10 \times \beta$. The indicator $\text{has}$ captures the extensive margin of disclosing any AI activity, separating whether a firm speaks from how it allocates its posture.
...
Because eight outcomes are estimated across six disclosure variables, hypothesis tests are evaluated using a Benjamini-Hochberg false discovery rate correction across the full family of 48 coefficients rather than outcome by outcome.
