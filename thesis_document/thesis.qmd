...
_KEY = [
    ...
    ("v", "intensity_expanding", "Cumulative intensity"),
    ("c", "Defensive vs. no posture", r"Defensive vs. none ($\beta_{\text{has}}$)"),
    ("c", "Vocal vs. Defensive",      r"Vocal vs. Def. ($\beta_{\text{voc}}$)"),
    ("c", "Governance vs. Defensive", r"Gov. vs. Def. ($\beta_{\text{gov}}$)"),
    ("c", "Governance vs. Vocal",     r"Gov. vs. Vocal ($\beta_{\text{gov}}-\beta_{\text{voc}}$)"),
]

def _panel(targets, letter):
    heads = [_bat[_bat["target"] == t]["label_outcome"].iloc[0] for t in targets]
    lines = [f"| **Panel {letter}** | " + " | ".join(heads) + " |",
             "|:---|" + "---:|" * len(targets)]
    for kind, key, label in _KEY:
        lines.append(f"| {label} | " + " | ".join(_mkt_cell(kind, key, t) for t in targets) + " |")
    lines.append("| Composition, joint $F$ | " + " | ".join(_joint_mkt_cell(t) for t in targets) + " |")
    lines.append("| $R^2$ | " + " | ".join(f"{float(_bat[_bat['target'] == t]['r2'].iloc[0]):.3f}" for t in targets) + " |")
    lines.append("| Calls | " + " | ".join(f"{int(_bat[_bat['target'] == t]['n_calls'].iloc[0]):,}" for t in targets) + " |")
    return lines + [""]

def _joint_mkt_cell(target):
...
Markdown("\n".join(_panel(("beta_post_63", "log_market_cap_post", "ncskew_post", "duvol_post"), "A")
                  + _panel(("gross_margin_change_pp", "roic_minus_wacc_change_pp",
                            "next_revenue_yoy_post", "ps_ratio_post"), "B")))
```
...
for panel, targets in zip("AB", _PANELS):
    ...
    for kind, key, label in _ROWS:
        _lines.append(f"| {label} | " + " | ".join(_mkt_cell(kind, key, t) for t in targets) + " |")
