...
The empirical sample is constructed from a fixed cohort of S&P 500 index constituents established as of January 1, 2021 ($N =$ `{python} uni_frame_total` unique corporate issuers, with dual-class share structures unified under the primary equity ticker). Holding index membership fixed at the start of the sample period eliminates survivorship and look-ahead bias: the panel retains `{python} uni_delisted` firms that subsequently underwent acquisition, restructuring, or delisting, tracking their disclosures throughout their active listing window rather than conditioning on ongoing index membership through 2026. Daily equity price series begin on January 1, 2020, guaranteeing a complete 252-trading-day history for market-model estimation ahead of early-2021 filings. Equity returns and trading volumes are gathered primarily from Yahoo Finance, falling back to Stooq and Tiingo for delisted constituents.
...
**Statutory filings and rate limits.** Mandatory regulatory disclosures are harvested directly from SEC EDGAR via automated routines built on `edgartools`, retrieving annual reports (Form 10-K), quarterly reports (Form 10-Q), proxy statements (DEF 14A), current reports (Form 8-K), and foreign issuer annual reports (Form 20-F). Queries strictly respect SEC fair-access protocols, throttling requests to under 10 queries per second with declared institutional user-agent headers. Forms are parsed into modular item sections following the SEC's standardized item taxonomy. Quarterly 10-Qs are treated as distinct reporting instruments and never pooled indiscriminately with comprehensive 10-K filings.

**Earnings call union and coverage audit.** Voluntary disclosure is tracked through quarterly earnings conference-call transcripts gathered from a deduplicated union across three providers: Hugging Face corporate earnings repositories, Equibles financial transcripts, and StockAnalysis. Because vendor feeds exhibit non-random omissions, transcripts undergo rigorous automated cross-verification against SEC filing dates, matching ticker symbols, primary company names, and fiscal period markers. Firm-quarters where transcript vendor coverage is completely absent (`{python} cov_call_gap_pct` of all panel firm-quarters) are designated as unmonitored (`cov_call_gap = 1`) and recorded as null rather than zero. This critical distinction prevents unobserved calls from being mistakenly analyzed as zero-disclosure communication.

**Observation window and 2026 adjustment.** Corporate documents span calendar filings and call dates from January 1, 2021 through `{python} cov_last_date`. Observations in 2026 reflect year-to-date activity through the cut-off date (`{python} cov_10k_2026` firms with Form 10-Ks, `{python} cov_10q_2026` firms with Form 10-Qs, and `{python} cov_calls_2026` firms with earnings calls). For multi-year trajectory comparisons and projected annual volumes, 2026 metrics are normalized using a same-calendar-window seasonal adjustment factor (`hd_act_factor`), computed as the ratio of full-year 2025 activity to activity recorded during the identical calendar span in 2025.

**External patent retrieval.** For non-textual validation of corporate technological capability, patent records covering 2021–2023 publications are retrieved from Google Patents via BigQuery. Patents are assigned to universe corporate issuers using curated assignee aliases (covering operating subsidiaries and IP holding entities) and classified using the OECD (2025) AI taxonomy, combining five unconditional core CPC groups with application groups filtered on AI keywords in the title or abstract.
...
Document segmentation splits ingested HTML and plain-text files into distinct paragraph instances. To eliminate non-textual noise, parsing removes XML tags, tabular financial matrices, ASCII graphics, and exhibits lacking conversational prose. Text blocks exceeding 3 characters and containing alphanumeric tokens are retained as scorable instances.

**Cryptographic deduplication.** In statutory SEC reporting, standard legal boilerplate, risk-factor language, and accounting notes frequently recur verbatim across subsequent quarters or exhibit exhibits. To prevent duplicated paragraphs from distorting downstream measures or incurring redundant LLM inference costs, each scorable paragraph is hashed using a 64-bit cryptographic BLAKE2b algorithm (`text_hash`). Downstream LLM extraction executes strictly over unique hashes; extracted frame and activity metadata are subsequently mapped back to all document instances sharing that hash.

**Supervised GBDT prefiltering gate.** To filter out non-AI corporate discourse prior to high-capacity language model evaluation, unique passages pass through a supervised Gradient Boosted Decision Tree (GBDT) classifier [@friedman2001]. The model scores 39 lexical, positional, and syntactic features, combining dense semantic anchor similarities (from `BAAI/bge-m3`), exact keyword stems, named entity frequencies, and document section tags. Calibrated at an intentionally low threshold ($\tau = 0.17$), the prefilter achieves $>98\%$ recall on held-out gold annotations (Appendix B), ensuring candidate AI disclosures are preserved while discarding the vast majority of non-technological narrative. Corporate documents containing zero prefiltered candidate paragraphs enter empirical panels as true zeros rather than missing records.
...
**Extraction model selection.** The production extraction model, `openai/gpt-5.6-luna` (hosted on Amazon Bedrock in `us-east-1`, operating with zero reasoning effort), was selected following an adversarial benchmark evaluating seven candidate model architectures on real 10-K corporate disclosure excerpts (@tbl-a0-judge-selection). Models were evaluated across internal field consistency, call latency, output fragmentation, and corpus-scale processing cost. `gpt-5.6-luna` exhibited zero logical field contradictions, predictable runtime (~3 seconds per paragraph), and competitive cost.
...
**Traceability and audit anchors.** To maintain end-to-end scientific auditability, every extracted semantic frame and disclosed activity is bound to an exact zero-indexed list of sentence citations within the source paragraph (`sentence_ids`). Any aggregate index, regression coefficient, or case excerpt in this study can be traced back to the exact sentence published in EDGAR or the conference-call transcript.
...
where $\text{rank}(\cdot) \in [0, 1]$ denotes the empirical percentile rank computed strictly within each reporting year among AI-disclosing firms. Large positive values characterize aggressive narrative claims with minimal verifiable backing ("AI washing"), whereas negative values denote extensive grounded implementation communicated with minimal promotional fanfare.

**Aggregation across documents.** Annual firm-year disclosure indicators are constructed by taking the **median document rate** across all corporate disclosures published by the firm during that fiscal period. The median dampens the impact of boilerplate legal exhibits, brief press releases, or outlier filings that would distort arithmetic means.

**Treatment of zeros and low-volume text.** Firms reporting no AI claims receive $D_{it} = 0$, $A_{it} = 0$, $G_{it} = 0$, and $S_{it} = 0$. Decoupling ($W_{it}$) is defined specifically over disclosing firms ($D_{it} > 0$), as rank divergence is informative only when narrative communication is present. For firms with sparse disclosures (e.g., 1–2 claims), empirical-Bayes shrinkage towards the sector-year prior is evaluated in Appendix B, confirming that ranking stability does not depend on low-denominator artifacts.
...
The data processing pipeline, feature engineering, and statistical analyses are implemented entirely in Python 3.12 within an isolated, deterministic environment managed via `uv`. The analytical stack relies on:

- **Data engineering:** `polars` for memory-efficient ingestion, text hashing, and multi-million-row Bronze and Silver Parquet transformations; `pandas` and `NumPy` for Gold analytical panels and tabular aggregations.
- **NLP and embeddings:** Dense semantic representations generated using the Hugging Face embedding model `BAAI/bge-m3` [@chen2024bge], supporting multi-lingual vectorization and 8,192-token context windows for semantic anchor scoring.
- **Machine learning and inference:** Supervised classification via `lightgbm` and `scikit-learn`; LLM extraction orchestrations powered by `openai/gpt-5.6-luna` accessed through Amazon Bedrock and OpenRouter with structured JSON outputs enforced by `pydantic-ai`.
- **Econometric modeling:** Cross-sectional and panel regressions, fixed-effects specifications, and survival diagnostics estimated using `statsmodels` and `SciPy`.
- **Reproducibility:** Complete code and pipeline artifacts version-controlled with Git; deterministic seeds applied across all stochastic steps.
