...
The unit of observation is a firm-date earnings call event ($i,t$) from 2021 to 2025 (transcripts on the same date are aggregated; zero-mention calls remain as valid zeros), modeled with an Analysis of Covariance (ANCOVA) specification that conditions each outcome directly on its own predetermined lag ($Y^{\text{pre}}_{i,t}$ at $t-1$) to avoid conflating unobserved firm heterogeneity with call impact. Industry-by-year fixed effects ($\text{SIC2} \times \text{Year}$) absorb industry-year shocks, standard errors are clustered by firm, and the disclosure vector — expanding historical posture ($\text{HistD}_{i,t^{-}}, \text{HistS}_{i,t^{-}}$), call-specific surprise ($\text{SurpriseD}_{i,t}, \text{SurpriseS}_{i,t}$), prior-year decoupling ($W_{i,t-1}$), and expanding-window archetype indicators ($\mathbf{1}_{\{\text{GovLed}\}_{i,t}}, \mathbf{1}_{\{\text{Vocal}\}_{i,t}}$, with *Defensive Disclosers* as reference; Section 3.3.3) — is estimated alongside predetermined controls for firm size, momentum, and operating fundamentals. Model $R^2$ is dominated by the lagged outcome by construction, so each table reports both the total and the partial $R^2$ of the disclosure block, which is the quantity of interest; the full equation, notation, and control list are given in Appendix E.
...
The results in @fig-economic-coefficients-c2 show an absence of operational premiums for disclosed substance: gross margin, R&D intensity, capital efficiency ($\text{ROIC}-\text{WACC}$), and systematic market risk all carry null coefficients on historical substance (@tbl-e-archetype-regressions). Disclosing granular operational activities does not confer short-term fundamental or risk advantages over peers.
...
Across the 14 fundamental and valuation tests ($2 \text{ dummies} \times 7 \text{ outcomes}$), no archetype coefficient survives Benjamini–Hochberg [@benjamini1995] FDR correction ($q^* = 0.05 / 14 \approx 0.0036$; @tbl-e-archetype-regressions). The uncorrected associations of *Governance-Led* with higher capital efficiency and lower beta reflect sectoral sorting into regulated sectors (commercial banking, insurance, and utilities) rather than an operational return from AI governance communication. When extending the test family to 18 outcomes with the tail-risk metrics of Section 6.5, only Governance-Led on crash risk survives.
...
The *Governance-Led* archetype, however, is associated with lower downside crash risk than Defensive Disclosers ($\beta = -0.150, p = 0.0016$ on $NCSKEW$; $\beta = -0.153, p = 0.0406$ on $DUVOL$) -- the sole estimate in the 18-test archetype family to clear Benjamini–Hochberg [@benjamini1995] FDR correction ($p \le 0.05/18 \approx 0.0028$), consistent with governance and structured risk disclosure mitigating bad-news withholding [@hutton2009; @kim2011; @au2023]. The result is an incremental post-call association over the lagged pre-call baseline, not a static sorting effect, and it holds under sector-exclusion and posture-switcher robustness checks reported in Appendix E.
...
Two checks probe whether the Governance-Led association with lower crash risk ($\beta = -0.150, p = 0.0016$ on $NCSKEW$; @fig-call-crash-risk, Section 6.5.1) reflects sector composition or a static sorting effect rather than the disclosure posture itself.

The first re-estimates the specification excluding Financial Services, Utilities, and Healthcare & Pharma -- the sectors carrying the heaviest regulatory risk-disclosure requirements. The Governance-Led coefficient is essentially unchanged ($\beta = -0.176, p = 0.021$), so the result is not an artifact of sector-specific regulation concentrating both Governance-Led firms and low crash risk in the excluded industries.

The second restricts the sample to firm-years that switch into or out of the Governance-Led archetype over the panel, rather than holding it throughout. The coefficient retains similar sign and magnitude ($\beta = -0.161, p = 0.063$), with a wider confidence interval reflecting the smaller switcher subsample. Because switchers are observed both inside and outside the archetype, this rules out a purely static sorting effect in which firms with permanently low crash risk simply happen to adopt Governance-Led posture.
