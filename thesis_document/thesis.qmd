...
This problem has become urgent since the release of ChatGPT, when artificial intelligence quickly became a standard topic in corporate disclosures [@anantharaman2026, pp. 1--2]. In the S&P 500, the share of firms mentioning AI in their SEC filings rose from `{python} f"{hd_share[2021]:.0f}"`% in 2021 to `{python} f"{hd_share[2025]:.0f}"`% in 2025, while narrative intensity across the same panel grew `{python} f"{hd_frames_ratio:.1f}"`-fold, reaching roughly `{python} f"{hd_int_ratio_est:.0f}"`-fold when extending through annualized 2026 projections. When nearly every firm mentions AI, counting words is no longer enough to distinguish operationally grounded disclosure from generic or promotional AI communication.
...
3. **A multi-dimensional measurement architecture.** Moving beyond keyword counts and simple sentiment dictionaries, the study builds a pipeline combining a tree-based classifier with zero-shot large language models into a six-construct measurement architecture that is reconciled across communication venues (earnings calls versus statutory filings). Because the architecture is modular and general, it serves as a reusable template to study corporate credibility in other narrative settings, such as ESG, cybersecurity, or future technology waves.
...
1. **Separating substance from boilerplate at scale.** With hundreds of firms repeating AI language every quarter, can an investor or analyst tell which mentions describe a named system, a deployment stage, or a quantified result, and which are undifferentiated promotional text, without reading every paragraph of every filing by hand?
2. **Weighting a claim by where it was made.** Should an investor or analyst treat the same AI initiative as equally credible when described on an earnings call and when repeated in the Form 10-K filed weeks later, or does promotional language run further ahead of verifiable evidence on the call than in the filing that carries statutory liability?
3. **Flagging decoupling without an internal audit.** Can the gap between how much a company talks about AI and how much grounded, verifiable activity it discloses be measured from public filings alone -- with no access to the company's internal AI program -- and used as a governance flag?
