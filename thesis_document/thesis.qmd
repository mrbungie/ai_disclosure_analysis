...
Baseline controls comprise market capitalization, R&D-to-sales, gross margin, return on assets (ROA), price-to-sales, forward revenue growth, market beta, and idiosyncratic volatility [@campbell2001; @ang2006]. ROA is preferred to margin- or turnover-based accounting controls because it is reported under a near-universal XBRL concept pair (net income, total assets), reaching 99.7% coverage of the firm-year panel, whereas single-tag ratios such as operating margin concentrate their missingness in regulated financial and insurance sectors. Because XBRL R&D coverage is 46%, this thesis includes a non-reporting indicator rather than imputing zero. Financial ratios are winsorized at the 1st and 99th percentiles (Appendix D).
...
This chapter evaluates how equity markets price corporate AI disclosures around earnings calls across S&P 500 constituents from 2021 through 2026.
...
_reg_all = pd.read_csv(find_path("data/processed/clusters/call_beta_regressions.csv"))
base_df = _reg_all[_reg_all["model"] == "baseline"].copy()
...
::: {custom-style="FigureNote"}
*Notes:* Standardized coefficients and 95% confidence intervals clustered by firm, with industry-by-year fixed effects ($\text{SIC2} \times \text{Year}$) and lagged pre-call beta ($N = 8,637$ calls, 446 firms, 316 fixed-effect cells, $R^2 = 0.628$).
:::

```{python}
#| echo: false
#| output: false
_samples_df = pd.read_csv(find_path("data/processed/clusters/call_beta_regression_samples.csv")).set_index("model")
_baseline_sample = {"n_calls": int(_samples_df.loc["baseline", "n_calls"]),
                     "n_firms": int(_samples_df.loc["baseline", "n_firms"]),
                     "n_fe_cells": int(_samples_df.loc["baseline", "n_fe_cells"]),
                     "r2": float(_samples_df.loc["baseline", "r2"])}
...
Across `{python} f"{_baseline_sample['n_calls']:,}"` calls and `{python} _baseline_sample['n_firms']` firms, historical disclosure intensity is positively associated with post-call market beta ($\beta =$ `{python} f"{hd_b:+.3f}"`, $p$ `{python} hd_pstr`), whereas single-call surprises ($\beta =$ `{python} f"{sd_b:+.3f}"`, $p$ `{python} sd_pstr`) and historical or single-call substantive operational activity ($\beta =$ `{python} f"{hs_b:+.3f}"`, $p$ `{python} hs_pstr`; $\beta =$ `{python} f"{ss_b:+.3f}"`, $p$ `{python} ss_pstr`) are statistically indistinguishable from zero (@fig-call-beta-baseline). This effect is stable across leave-one-year-out re-estimations and archetype-conditional specifications (Appendix E).
...
_gen_c1 = pd.read_csv(find_path("data/processed/clusters/call_beta_generalized_targets.csv"))
_beta_c1 = pd.read_csv(find_path("data/processed/clusters/call_beta_regressions.csv"))
_beta_c1 = _beta_c1[_beta_c1["model"] == "baseline"].copy()
_beta_c1["target"] = "beta"
_res_c1 = pd.concat([_gen_c1, _beta_c1[["target", "variable", "label", "beta_std", "ci95_low", "ci95_high", "p"]]], ignore_index=True)
...
As shown in @fig-economic-coefficients-c1, historical disclosure intensity predicts larger market cap ($\beta = +0.086, p < 0.001$) and faster revenue growth ($\beta = +0.147, p = 0.014$); its association with price-to-sales is directionally positive but only marginal ($\beta = +0.013, p = 0.055$). Markets price persistent narrative volume through firm size and growth expectations, supporting $H_{6a}$ for scale and growth while leaving its valuation-multiple channel weaker than for beta.
...
_res_c2 = pd.read_csv(find_path("data/processed/clusters/call_archetype_full_battery_targets.csv"))
_res_c2["target"] = _res_c2["target"].str.replace("_post_63", "", regex=False).str.replace("_post", "", regex=False)
_res_c2["ci95_low"] = _res_c2["beta_std"] - 1.96 * _res_c2["se"]
_res_c2["ci95_high"] = _res_c2["beta_std"] + 1.96 * _res_c2["se"]
...
Communication archetypes likewise offer no operational premium once continuous narrative volume and substance are held constant. *Governance-Led* carries a null coefficient across all seven economic outcomes ($p \ge 0.13$; @tbl-e-archetype-regressions). *Vocal Substantives* is null on six of the seven outcomes and only marginally associated with lower gross margin ($\beta = -0.041, p = 0.046$), indicating that promotional commercial positioning does not earn fundamental rewards ($H_{6b}'$).

Across the 14 fundamental and valuation tests ($2 \text{ dummies} \times 7 \text{ outcomes}$), no archetype coefficient survives Benjamini–Hochberg [@benjamini1995] FDR correction ($q^* = 0.05 / 14 \approx 0.0036$; @tbl-e-archetype-regressions); the smallest uncorrected p-value, Vocal Substantives on gross margin, is an order of magnitude short of the threshold at its rank. Extending the test family to 18 outcomes with the tail-risk metrics of Section 6.5 does not change this verdict: none of the archetype coefficients, including Governance-Led on crash risk, clears multiplicity correction (Appendix E).
...
This section evaluates Channel 3 ($H_{6c}$): whether narrative decoupling ($W = \text{pctrank}(D) - \text{pctrank}(S)$) represents cheap talk and opacity. Theoretical models of cheap talk and information opacity [@crawford1982; @jin2006] predict that narrative decoupling should not alter market-wide covariance ($\beta$) but should instead manifest, if at all, in firm-specific tail risk; @fig-call-crash-risk reports both tests directly.
...
_crash_df = pd.read_csv(find_path("data/processed/clusters/call_crash_archetype_regressions.csv"))
_crash_samples = pd.read_csv(find_path("data/processed/clusters/call_crash_archetype_regression_samples.csv")).set_index("target")
_sub_nc = _crash_df[_crash_df["target"] == "ncskew_active_dummies"].copy()
...
::: {custom-style="FigureNote"}
*Notes:* Standardized coefficients and 95% confidence intervals clustered by firm, with industry-by-year fixed effects ($\text{SIC2} \times \text{Year}$) and lagged pre-call crash risk ($N = 8,642$ calls across 428 firms, $R^2 = 0.113$; sample includes firms with no qualifying AI-disclosure history via a fourth, unreported "No AI" indicator). The Governance-Led indicator ($\beta = -0.124, p = 0.046$) is highlighted in rust; it clears the conventional 5% threshold uncorrected but does not survive Benjamini–Hochberg FDR correction across the pre-specified 18-test archetype family ($p \le 0.05/18 \approx 0.0028$ at its rank).
:::

The crash risk estimates in @fig-call-crash-risk yield two findings. Historical disclosure intensity is associated with *lower* crash risk ($\beta = -0.049, p = 0.029$ on $NCSKEW$), the opposite direction from its positive association with post-call beta in Channel 1: the same persistent narrative visibility that raises systematic exposure also coincides with fewer idiosyncratic negative-return surprises. Continuous decoupling shows no short-run association with crash risk over a 6-month horizon ($\beta = +0.006, p = 0.666$), running counter to an immediate reading of $H_{6c}$; whether bad news instead accumulates over longer, multi-year horizons remains an open extension.

The *Governance-Led* archetype carries a negative, nominally significant coefficient on $NCSKEW$ relative to Defensive Disclosers ($\beta = -0.124, p = 0.046$), directionally consistent with governance and structured risk disclosure mitigating bad-news withholding [@hutton2009; @kim2011; @au2023], and a similarly signed but non-significant coefficient on $DUVOL$ ($\beta = -0.132, p = 0.051$). Neither clears Benjamini–Hochberg [@benjamini1995] FDR correction across the pre-specified 18-test archetype family: the smallest p-value in that family (Vocal Substantives on gross margin, $p = 0.046$) already falls short of the rank-1 threshold ($q^* \approx 0.0028$), so Governance-Led on crash risk clears no rank either. The association also attenuates below conventional significance excluding Financial Services, Utilities, and Healthcare & Pharma ($\beta = -0.075, p = 0.416$) and restricting the sample to firms that move into or out of the archetype over the panel ($\beta = -0.060, p = 0.287$; Appendix E reports an additional robustness check). Taken together, the Governance-Led/crash-risk association is a directionally consistent but fragile result: nominally significant only before correcting for the size of the test family, and not robust to sector composition or to isolating within-firm posture changes.

Historical disclosure intensity's negative association with crash risk qualifies @yang2026, who find that AI adoption reduces crash risk in Chinese listed firms via internal-control quality: in this S&P 500 setting it is not disclosed AI substance but persistent AI narrative volume itself -- the same construct priced in systematic beta -- that coincides with lower downside tail risk, while the Governance-Led disclosure configuration is only a suggestive, not a robust, complement to that pattern.
...
|:---|:---|:---|
| **$H_{6a}$: Thematic Factor Exposure ($D$)** | Supported as cross-sectional association, null as call impact ($\beta =$ `{python} f"{hd_b:+.3f}"`, $p$ `{python} hd_pstr` post-call; pre-call placebo already $\beta = +$`{python} f"{rob_placebo_d['beta_std']:.3f}"`, nearly double the post-call estimate; market cap $\beta = +0.086$; revenue growth $\beta = +0.147$) | Firms with persistently higher narrative volume carry higher systematic exposure to the AI theme; the earnings call itself does not move that exposure. |
| **$H_{6b}$: Costly Signaling ($S$)** | Not supported (gross margin $p = 0.172$; ROIC$-$WACC $p = 0.544$; beta $p = 0.401$) | Disclosed operational depth earns no valuation premium or risk discount in the short run. |
| **$H_{6c}$: Tail Risk / Opacity ($W$)** | Not supported for continuous $W$ ($p = 0.666$); Governance-Led nominally significant uncorrected ($\beta = -0.124, p = 0.046$) but fails FDR correction and sector-exclusion/posture-switcher checks | Narrative decoupling does not predict 6-month crashes. Governance-focused disclosure's association with lower downside tail risk is suggestive, not robust. |
...
_cb = pd.read_parquet(_p("call_beta_main_panel_10k10q_asof.parquet"), columns=["ticker", "fecha", "beta_post_63"])
...
samples = pd.read_csv(find_path("data/processed/clusters/call_beta_regression_samples.csv")).set_index("model")
...
_arch_full = pd.read_csv(find_path("data/processed/clusters/call_archetype_full_battery_targets.csv"))

_arch_targets = [
    ("beta_post_63", "Market Beta (post)"),
    ("log_market_cap_post", "Log Market Cap"),
    ("rd_intensity_post", "R&D / Sales"),
    ("gross_margin_post", "Gross Margin"),
    ("ps_ratio_post", "Price-to-Sales"),
    ("next_revenue_yoy_post", "Revenue Growth (t+1)"),
    ("roic_minus_wacc_post", "ROIC - WACC"),
]
...
for key, label in _arch_targets:
    ...
    arch_reg_rows.append([
        ...
        _arch_fmt(sub, "dum_gov"), _arch_fmt(sub, "dum_voc"),
        _arch_fmt(sub, "hist_disclosure"), _arch_fmt(sub, "hist_substance"),
    ])
...
::: {custom-style="FigureNote"}
*Notes:* Standardized coefficients with $p$-values in parentheses, standard errors clustered by firm, industry-by-year fixed effects ($\text{SIC2} \times \text{Year}$). Continuous controls include the outcome's own pre-call level, 60-day return momentum, and return on assets (ROA) (and log market cap where not the target). Partial R² (AI + arch) measures the incremental variance explained by adding the continuous AI signals and strategic archetype indicators over baseline controls and fixed effects. Across the 14 archetype tests on economic fundamentals ($2 \text{ dummies} \times 7 \text{ outcomes}$), the Benjamini--Hochberg [@benjamini1995] FDR critical threshold at rank 1 is $q^* = 0.05 \times 1/14 \approx 0.0036$; the smallest uncorrected p-value in this family (Vocal Substantives on gross margin, $p = 0.046$) falls short of that threshold, so no coefficient clears multiplicity correction. Extending the family to 18 tests with the tail-risk outcomes of Section 6.5.2 ($q^* = 0.05 \times 1/18 \approx 0.0028$) does not change this: Governance-Led on crash risk ($NCSKEW$, $p = 0.046$) also fails to clear its rank.
:::
