...
When S&P 500 corporations discuss artificial intelligence, which suppliers do they name? Commercial names alone answer it -- companies, cloud platforms, accelerators, model families and enterprise AI products, read inside paragraphs that already carry an extracted AI disclosure frame (`configs/ai_vendor_ecosystems.yaml`, Appendix C). Country words are excluded, since "China" locates a market rather than a technology; ambiguous names need corroboration in the same paragraph, so "copilot" requires Microsoft or GitHub; and a firm naming its own products does not count, since the measure is the reach of external suppliers. Presence is binary per document and ecosystems overlap. The **AI technology stack** (model developers, cloud infrastructure, semiconductors) and **enterprise software** (applied AI inside business platforms) are reported apart, since a blended number would hide which of the two is moving.
