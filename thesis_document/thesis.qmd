...
| Theoretical Channel | Expected Outcome | Hypothesis & Status |
|:---|:---|:---|
| **Channel 1: Thematic Factor Exposure ($D$)** | Higher systematic risk ($\beta$) and valuation multiples [@diamond1991; @eisfeldt2023] | $H_{6a}$: Factor exposure (supported) |
| **Channel 2: Costly Signaling ($S$)** | Stronger fundamentals (gross margin, $\text{ROIC}-\text{WACC}$) and lower risk [@spence1973; @verrecchia1983] | $H_{6b}$: Costly signaling (not supported) |
| **Channel 3: Narrative Decoupling ($W$)** | Idiosyncratic tail risk ($NCSKEW$), no systematic $\beta$ impact [@crawford1982; @jin2006] | $H_{6c}$: Tail risk / opacity (not supported; exploratory: Governance-Led) |
...
Robustness tests reported in @tbl-call-beta-paper (Appendix E) confirm that the historical disclosure effect is not an artifact of model specification. The association survives point-in-time accounting controls (ROA and leverage; $\beta = 0.076, p = 0.015$), a full-year post-call window (+252 days; $\beta = 0.073, p = 0.011$), and the Fama–French 3-factor risk benchmark ($\beta = 0.063, p = 0.021$). Under firm fixed effects, historical disclosure attenuates ($\beta = 0.100, p = 0.070$), while first differences ($\Delta\beta$) yield an attenuated estimate with wide confidence intervals ($\beta = 0.056, p = 0.095$). Combined with pre-call placebo tests ($\beta^{\text{pre}} \sim D$ yields $\beta = +0.190, p < 0.001$, while substance is null), this pattern indicates cross-sectional sorting into thematic risk: firms establishing high narrative visibility carry higher systematic beta, but quarterly changes in disclosure volume do not move beta over time.
...
We evaluate the tail risk channel by estimating Negative Coefficient of Skewness ($NCSKEW$) and Down-to-Up Volatility ($DUVOL$) over $[+21, +126]$ trading days post-call (@fig-call-crash-risk).
...
Second, the indicator for *Governance-Led Disclosers* exhibits a statistically significant negative association with downside crash risk relative to Defensive Disclosers ($\beta = -0.150, p = 0.0016$ on $NCSKEW$; $\beta = -0.153, p = 0.0406$ on $DUVOL$). Across the full 18-test family of archetype hypotheses, this $NCSKEW$ coefficient is the sole estimate to clear Benjamini–Hochberg [@benjamini1995] FDR multiplicity correction ($p = 0.0016 \le 0.05 \times 1/18 \approx 0.00278$). While coherent with theoretical accounts where governance and risk disclosure mitigate bad-news withholding [@hutton2009; @kim2011], three robustness checks clarify the identification behind this result:
...
|:---|:---|:---|
| **$H_{6a}$: Thematic Factor Exposure ($D$)** | Supported ($\beta = +0.079, p = 0.006$; market cap $\beta = +0.124$; P/S $\beta = +0.177$) | Markets price persistent narrative volume as systematic exposure to the AI theme. |
...
Two structural limits qualify these market findings. First, pre-call beta placebos show positive associations ($\beta^{\text{pre}} \sim D$ yields $\beta = +0.190, p < 0.001$, while substance is null), indicating cross-sectional sorting into thematic risk rather than call-event impact. Second, multiple calls from the same firm map to the same annual 10-K, inducing pseudo-replication in accounting fundamentals that warrants caution for low-frequency fundamental outcomes.

Because equity markets reward promotional talk without filtering for operational depth, market prices alone cannot discipline corporate narrative. This decoupling gap makes internal due diligence essential, motivating the credibility framework presented next.

# From Measurement to Managerial Application

The central empirical finding of Chapter 6 is an asymmetry: equity markets price narrative disclosure intensity ($D$) as thematic factor risk, but fail to price verifiable operational substance ($S$) or penalize cheap talk in the short run. Because market prices cannot filter rhetoric from execution, investment allocators and corporate directors cannot rely on market multiples to discipline AI disclosure. To bridge this analytical gap, this chapter operationalizes our measurement architecture into a concrete credibility framework for due diligence, corporate governance, and regulatory oversight.

The framework translates the measurement chain into five diagnostic pillars: narrative volume ($D$), claimed operational activities ($A$), evidentiary grounding ($G$), substance ($S$), and decoupling ($W$), evaluated alongside strategic posture ($P$) and cross-venue reconciliation between voluntary conference calls and statutory Form 10-K filings.

## Applying the framework: worked corporate examples

We now run the framework on contrasting firm-years from the S&P 500 (@tbl-firm-comparisons). Corporate AI communication evolves across reporting cycles, making specific firm-years the natural unit for evaluating how disclosure volume, evidentiary grounding, and cross-venue consistency interact in practice.
...
The framework organizes due diligence through a 2×2 decision heuristic built on Disclosure Intensity ($D$) and Disclosed Substance ($S$) (@tbl-screening-matrix). The decoupling score $W$ does not serve as a fraud classifier ($W \neq \text{Legal Wrongdoing}$); instead, $D$ and $S$ determine the action required in each quadrant, while $W$ guides the depth of investigation within it.
...
|---|---|---|
| **High Disclosure Intensity ($D$)** | **INVESTIGATE** (`{python} scr["investigate"]` firms, `{python} f"{scr_pct['investigate']:.1f}"`%)<br>Audit claims and verify whether oral conference-call claims are reflected in statutory Form 10-Ks ($W \gg 0$). | **VALIDATE** (`{python} scr["validate"]` firms, `{python} f"{scr_pct['validate']:.1f}"`%)<br>Confirm documented evidence remains cross-venue consistent and operational grounding is sustained ($W \approx 0$). |
| **Low Disclosure Intensity ($D$)** | **DEPRIORITIZE** (`{python} scr_deprior_total` firms, `{python} f"{scr_deprior_total_pct:.1f}"`% of index)<br>Low narrative exposure with minimal reported activity; requires no active diligence unless posture shifts ($W \approx 0$). | **EXPLORE** (`{python} scr["explore"]` firms, `{python} f"{scr_pct['explore']:.1f}"`%)<br>Investigate quiet execution: concrete, grounded operational capability deployed under minimal promotional fanfare ($W \ll 0$). |

: AI Disclosure Decision Heuristic: portfolio screening by disclosure intensity ($D$) and substance ($S$) across the S&P 500 in the 2026 reporting season (`{python} scr_n_index` index firms; `{python} scr_n_disc` disclosing issuers). {#tbl-screening-matrix}

### Operational portfolio screens

Beyond the four primary quadrants in @tbl-screening-matrix, two distributional cuts sharpen due-diligence triage: within the Investigate quadrant, `{python} scr_tail` FY2026 firm-years sit above the 95th percentile of $W$ in the full 2021–2026 panel (Extreme Decoupling), while `{python} scr_neg` disclosers exhibit negative $W$ (substance rank exceeding narrative volume rank, the pool anchoring the Explore cell).

A fifth cross-cutting screen, Reconcile, focuses on cross-venue credibility among the `{python} scr_n_dual` issuers with both a 2026 earnings call and a 2026 filing. Of these, `{python} scr_n_gap` display an oral-minus-filing promotional gap exceeding 1.5 standard deviations above the 2026 mean ($Z_{\text{gap}} > 1.5$), mirroring the pre-scrutiny Welltower pattern.
...
| **2. Evidentiary Grounding** | Anchor claims with named products, proprietary tools, and quantified operational metrics (costs, latency, revenue). | Look for falsifiable metrics; generic buzzwords reflect ceremonial rhetoric rather than execution. |
...
| **4. Cross-Venue Consistency** | Reconcile voluntary conference-call remarks against signed Form 10-K filings under statutory legal liability. | Scrutinize promotional claims that disappear from 10-Ks; cross-venue divergence invites regulatory inquiry. |
| **5. Supervision & Governance** | Institutionalize board and audit committee oversight of AI risk in proxy statements (DEF 14A) and Item 10. | Board oversight disclosures are associated with significantly lower downside tail risk ($NCSKEW$), mitigating opacity. |
...
**Heterogeneity.** Firms sort into three stable communication archetypes shaped by industry norms, and posture predicts disclosed activity beyond narrative volume: at equal AI text volume, Vocal Substantives report `{python} f"{hd_vs_ratio:.2f}"` times as many activities as Defensive Disclosers, yet once historical disclosure intensity is controlled for, archetype membership adds no incremental explanatory power for firm valuation or risk: what market profiles track is narrative volume, not strategic posture.
