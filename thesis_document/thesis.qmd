...
The unit of observation is a firm-date earnings call event ($i,t$) from 2021 to 2025 (transcripts on the same date are aggregated; zero-mention calls remain as valid zeros). Because asset prices, beta, and fundamentals follow persistent martingale processes ($\mathbb{E}[Y^{\text{post}}_{i,t} \mid \mathcal{F}_{t^{-}}] \approx Y^{\text{pre}}_{i,t}$), static regressions conflate unobserved firm heterogeneity with call impact. This thesis uses a unified Analysis of Covariance (ANCOVA) conditioning directly on the outcome's own predetermined lag ($Y^{\text{pre}}_{i,t}$ at $t-1$):
