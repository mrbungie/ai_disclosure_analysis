...
Four limitations qualify how far these findings travel. First, disclosed activity reflects corporate assertions rather than an internal engineering census, not confirmed operational implementation; AI patents provide an independent benchmark (Appendix B), though patents track technological capacity rather than operational deployment, and the five grounding checks underlying the disclosure construct were fixed before the generative-AI era, so they may undervalue disclosure patterns specific to it, such as naming a third-party model provider without a proprietary product or metric. Second, market associations are non-causal, cross-sectional evidence, with the one instance of direct regulatory scrutiny a single case rather than a general econometric test. Third, the sample is the S&P 500, so these patterns describe large, closely covered U.S. issuers and may not extend to smaller or less-scrutinized firms. Fourth, the 2026 figures cited throughout are year-to-date, projected to full-year via the same-window seasonal adjustment described where they first appear, and should be read as provisional until the year closes.
