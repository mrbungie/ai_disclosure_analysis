...
Only `{python} f"{c5 / c1 * 100:.1f}"`% of disclosed AI activities reach the top of the ladder, pairing an operational deployment with a quantified performance metric (@fig-ladder-substance). The attrition across stages is steep: while `{python} f"{c2 / c1 * 100:.1f}"`% name a business function and `{python} f"{c3 / c1 * 100:.1f}"`% describe deployed or scaled systems, the share identifying a concrete product, workflow, or falsifiable metric falls to `{python} f"{c4 / c1 * 100:.1f}"`%. For documentary review, these criteria translate directly into audit points: companies can pinpoint what factual backing is missing from their claims, while investors can target what evidence to request, without demanding that every initiative simultaneously satisfy all attributes.

## More activities, thinner grounding

The generative AI boom multiplied the volume of disclosed activities while diluting their average grounding (@fig-volume-vs-grounding). Following the 2023 inflection, aggregate activity claims expanded several-fold, yet the proportion of activities in production, naming specific tools, or tied to concrete business functions declined steadily. As predicted by institutional theory [@meyer1977], once AI terminology became an external market expectation, rhetorical adoption diffused far faster than underlying organizational execution. The same widening appears in the two prior findings this pattern speaks to: @anantharaman2026 document the post-ChatGPT surge in corporate AI discussion without separating what firms claim to run from what they claim to plan, and @basnet2025 show that separating actionable from speculative AI text is what makes disclosure informative. The measurement here supplies the missing half of the first and extends the second from a binary split to a graded account of what an activity names.[^fn-seasonal-46]
...
Disclosed activities in 2025 were `{python} f"{growth_ratio:.1f}"` times the 2021 count (projected to `{python} f"{proj_ratio:.1f}"` times in 2026), but the marginal post-ChatGPT disclosure was markedly less concrete and more speculative. Over 2021–2025, the share of initiatives at a deployed or scaled stage fell from `{python} f"{dep_share[main_years[0]]:.0f}"`% to `{python} f"{dep_share[main_years[-1]]:.0f}"`% (`{python} f"{dep_share[2026]:.0f}"`% in 2026 YTD), while those naming a specific product or process dropped from `{python} f"{named_prod[main_years[0]]:.0f}"`% to `{python} f"{named_prod[main_years[-1]]:.0f}"`% (`{python} f"{named_prod[2026]:.0f}"`% in 2026). Growth in project counts must therefore be evaluated alongside evidentiary grounding, as expanding initiative volume alone does not indicate operational progress.
...
Disclosing issuers consistently prioritize proprietary branding over external attribution. However, applying an internal brand name does not identify who developed the underlying model. For reporting companies, credibility improves by clarifying what is built in-house versus integrated from external platforms; for investors, it signals the need to examine model provenance, vendor reliance, and architectural dependencies directly.
