...
::: {custom-style="FigureNote"}
*Notes:* Monthly open-source mentions in earnings calls (Jan 2021–Feb 2026). The model controls for month seasonality and reporting volume; the shaded band is the 95% confidence interval and the dashed line represents the counterfactual pre-event trajectory.
:::


Open-source vocabulary in earnings calls rises sharply at the turn of 2025 and then turns down (@fig-deepseek-break). Over `{python} str(_dsj["months"])` months of calls, the estimated step is `{python} f"{_dsj['step'] * 1e2:+.2f}"` mentions per 100,000 words (95% CI `{python} f"[{_dsj['step_ci_low'] * 1e2:+.2f}, {_dsj['step_ci_high'] * 1e2:+.2f}]"`, $p$ `{python} pstr(_dsj["p_step"])`), against a monthly mean of `{python} f"{_dsj['mean_before'] * 1e2:.2f}"` before and `{python} f"{_dsj['mean_after'] * 1e2:.2f}"` after. January itself reaches `{python} f"{float(_ds.loc[_ds['month'] == '2025-01', 'rate_per_1k'].iloc[0]):.2f}"`, roughly ten times the earlier mean, and the level is back near it within two quarters.

The firmer result is the change in direction. The series had been rising at `{python} f"{_dsj['pre_trend'] * 1e2:+.3f}"` per month ($p$ `{python} pstr(_dsj["p_pre_trend"])`); afterwards the slope is negative, `{python} f"{_dsj['post_slope'] * 1e2:+.3f}"` (95% CI `{python} f"[{_dsj['post_slope_ci_low'] * 1e2:+.3f}, {_dsj['post_slope_ci_high'] * 1e2:+.3f}]"`, $p$ `{python} pstr(_dsj["p_post_slope"])`), and the later path ends below a simple continuation of the earlier one. A conversation that had grown for four years turns down after the episode that made it salient; whether it falls faster than it rose the data do not settle ($p$ `{python} pstr(_dsj["p_steeper_than_rise"])`).
