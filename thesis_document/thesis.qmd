...
To determine the number of archetypes $k$, this thesis evaluates reproducibility across $k \in \{2, 3, 4, 5\}$ under two non-parametric bootstrap regimes with 50 independent replicates (@fig-archetype-stability): (1) frame-level resampling within firms (evaluating measurement error), and (2) firm-level resampling (evaluating sample composition sensitivity). All archetypes are required to clear a mean Jaccard overlap floor of $\ge 0.60$ under both regimes.
...
_reg_all = pd.read_csv(results("call_beta", "call_beta_regressions.csv"))
base_df = _reg_all[_reg_all["model"] == "baseline"].copy()
# The call regressors (scripts/analytics/call_regression.py): the AI block and the
# three archetype weights, labelled as in the results.
AI_LABELS = {"intensity_expanding": "Cumulative disclosure intensity", "hist_w": "Historical decoupling (HistW)",
             "w": "Call decoupling (W)", "w_voc": "Vocal Substantives weight",
             "w_gov": "Governance-Led weight", "w_def": "Defensive weight"}
base_df["label"] = base_df["variable"].map(AI_LABELS)
ai_vars = list(AI_LABELS)
base_ai = base_df[base_df["variable"].isin(ai_vars)].copy()

ai_vars_plot = list(AI_LABELS.items())
...
#| echo: false
#| output: false
_samples_df = pd.read_csv(results("call_beta", "call_beta_regression_samples.csv")).set_index("model")
...
int_b, int_p, int_pstr = _bp("intensity_expanding")
hw_b, hw_p, hw_pstr = _bp("hist_w")
w_b, w_p, w_pstr = _bp("w")
voc_b, voc_p, voc_pstr = _bp("w_voc")

def _sig(p):
    return "positive and significant" if p < 0.05 else "not statistically significant"
```

Across `{python} f"{_baseline_sample['n_calls']:,}"` calls and `{python} _baseline_sample['n_firms']` firms, historical disclosure intensity is positively associated with post-call market beta ($\beta =$ `{python} f"{int_b:+.3f}"`, $p$ `{python} int_pstr`), whereas single-call surprises ($\beta =$ `{python} f"{w_b:+.3f}"`, $p$ `{python} w_pstr`) and historical or single-call substantive operational activity ($\beta =$ `{python} f"{hw_b:+.3f}"`, $p$ `{python} hw_pstr`; $\beta =$ `{python} f"{voc_b:+.3f}"`, $p$ `{python} voc_pstr`) are statistically indistinguishable from zero (@fig-call-beta-baseline). This effect is stable across leave-one-year-out re-estimations and archetype-conditional specifications (Appendix E).
...
_gen_c1 = pd.read_csv(results("call_beta", "call_beta_generalized_targets.csv"))
_beta_c1 = pd.read_csv(results("call_beta", "call_beta_regressions.csv"))
...
SERIES_C1 = [
    ("intensity_expanding", "Cumulative intensity", "#8EA7A3"),
    ("hist_w", "HistW", "#B85F42"),
    ("w", "W", "#D5B1A5"),
]
...
_rob_acct_df = pd.read_csv(results("call_beta", "call_beta_regressions.csv"))
_rob_ff3_df = pd.read_csv(results("call_beta", "call_beta_robustness_market_model.csv"))
_rob_win_df = pd.read_csv(results("call_beta", "call_beta_robustness_windows.csv"))
_rob_fe_df = pd.read_csv(results("call_beta", "call_beta_robustness_firm_fe.csv"))
_rob_delta_df = pd.read_csv(results("call_beta", "call_beta_robustness_delta_beta.csv"))
_rob_placebo_df = pd.read_csv(results("call_beta", "call_beta_robustness_placebo.csv"))

rob_acct = _rob_acct_df[(_rob_acct_df["model"] == "accounting_M1") & (_rob_acct_df["variable"] == "intensity_expanding")].iloc[0]
rob_ff3 = _rob_ff3_df[(_rob_ff3_df["block"] == "ff3") & (_rob_ff3_df["variable"] == "intensity_expanding")].iloc[0]
rob_win252 = _rob_win_df[(_rob_win_df["block"] == "beta_window[post_252]") & (_rob_win_df["variable"] == "intensity_expanding")].iloc[0]
rob_fe = _rob_fe_df[_rob_fe_df["variable"] == "intensity_expanding"].iloc[0]
rob_delta = _rob_delta_df[_rob_delta_df["variable"] == "intensity_expanding"].iloc[0]
rob_placebo_d = _rob_placebo_df[(_rob_placebo_df["block"] == "placebo_pre_beta") & (_rob_placebo_df["variable"] == "intensity_expanding")].iloc[0]
```
...
_res_c2 = pd.read_csv(results("crash_archetypes", "call_archetype_full_battery_targets.csv"))
...
SERIES_C2 = [
    ...
    ("w", "W", "#D5B1A5"),
    ("intensity_expanding", "Cumulative intensity", "#8EA7A3"),
    ("w_gov", "Governance-Led weight", "#263B4A"),
    ("w_def", "Defensive weight", "#355F6B"),
]
...
_crash_df = pd.read_csv(results("crash_archetypes", "call_crash_archetype_regressions.csv"))
_crash_samples = pd.read_csv(results("crash_archetypes", "call_crash_archetype_regression_samples.csv")).set_index("target")
_sub_nc = _crash_df[_crash_df["target"] == "ncskew_weights"].copy()

order = ["w_gov", "w_def", "w_voc", "intensity_expanding", "hist_w", "w"]
labels = {
...
colors = ["#B85F42" if v == "w_gov" else "#73828C" for v in order]
...
|:---|:---|:---|
| **$H_{6a}$: Thematic Factor Exposure ($D$)** | Supported as cross-sectional association, null as call impact ($\beta =$ `{python} f"{int_b:+.3f}"`, $p$ `{python} int_pstr` post-call; pre-call placebo already $\beta = +$`{python} f"{rob_placebo_d['beta_std']:.3f}"`, nearly double the post-call estimate; market cap $\beta = +0.086$; revenue growth $\beta = +0.147$) | Firms with persistently higher narrative volume carry higher systematic exposure to the AI theme; the earnings call itself does not move that exposure. |
...
Disclosed AI activity predicts AI patenting even after controlling for firm size and sector: with `{python} n_b1` firms across `{python} n_sic_b1` SIC groups, a one-log-point increase in disclosed AI activities is associated with `{python} f"{_coef_act_b1:.2f}"` additional log-patents (p < 0.001), and the same holds for grounded activities alone (`{python} f"{_coef_grd_b1:.2f}"`, p < 0.001). These relationships confirm that LLM-derived disclosure constructs align with an independent, objective measure of technological development.
...
rows.append(["N",
             f"{_base_n:,}",
             f"{_acct_n:,}",
             f"{int(ff3_df.loc['w', 'n_calls']):,}" if "n_calls" in ff.columns else "—",
             f"{int(win252.loc['w', 'n_calls']):,}" if "n_calls" in win.columns else "—",
             f"{int(fe_df.loc['w', 'n_calls']):,}" if "n_calls" in fe.columns else "—",
             f"{int(delta_df.loc['w', 'n_calls']):,}" if "n_calls" in delta.columns else "—"])
...
for key, label in _arch_targets:
    ...
    arch_reg_rows.append([
        ...
        _arch_fmt(sub, "w_gov"), _arch_fmt(sub, "w_def"), _arch_fmt(sub, "w_voc"),
        _arch_fmt(sub, "intensity_expanding"), _arch_fmt(sub, "hist_w"),
    ])

headers_arch = ["Target Outcome", "N Calls", "Firms", "R²", "Partial R² (AI + arch)", "Governance-Led weight", "Defensive weight", "Vocal weight", "Cumulative intensity", "HistW"]
