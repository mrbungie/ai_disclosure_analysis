...
Five findings anchor the dissertation. First, the composition of AI disclosure changed more than its quantity: between 2021 and 2025 risk language grew twenty-two-fold and governance language thirteen-fold, while promotional language fell from 11% to 5% of AI statements in filings. Second, firms sort into three reproducible disclosure archetypes, Product Deployers, Governance Adopters and Risk Listers, plus a fourth group that does not disclose AI at all; the archetypes separate firms on R&D intensity, margins, beta and valuation they were never built to predict, and Product Deployers remain distinct within their own sector and year. A second pass over 17,420 behavioural paragraphs gives the archetypes tangible content: 27,583 disclosed AI activities show that Deployers sell their own branded AI, invest in infrastructure and name providers, customers and figures, Governance Adopters deploy unbranded own models inward, and Risk Listers describe almost nothing concrete. Third, and centrally, the *content* of AI disclosure adds explanatory power over fundamentals and disclosure volume for beta, idiosyncratic volatility, price-to-sales, R&D intensity and next-year revenue growth: two to four points of partial R², confirmed by bootstrap, a joint Wald test and a permutation test within sector-year. The dimension that carries the signal is specificity, and a second pass that turns behavioural statements into concrete disclosed activities (what is deployed, for which function, with which provider) shows that the signal comes both from the identifiable activity and from how it is documented. Promotional tone carries none. Fourth, the SEC's 2024 scrutiny cannot be separated from the generative-AI boom it landed in: more and less exposed firms were already diverging before the regulator spoke, and every design that compares them fails its pre-trend test. Fifth, the one design that removes the boom, comparing the same firm in the same fiscal year on its earnings call and in its filings, shows that firms put twenty times more promotional AI claims per paragraph in front of analysts than in what they file, that this gap tripled between 2022 and 2025, and that it did not narrow when the regulator announced it would look for exactly that. In activities, the two venues describe the same mix of deployments; the call adds the figures, the business functions, the providers and the product names.
...
| Stage | Filter | Unit | 10-K | 10-Q | DEF 14A | 8-K | Calls | Total |
|---|---|---|---:|---:|---:|---:|---:|---:|
| **A. Corpus** | | | | | | | | |
| Documents | rule: S&P 500 universe × EDGAR / transcripts | documents | 2,873 | 8,099 | 2,646 | 34,416 | 7,947 | 55,981 |
| Scorable paragraphs | rule: >3 characters with an alphanumeric | instances | 1,422,746 | 1,593,351 | 3,254,803 | 1,529,705 | 553,942 | 8,354,547 |
| Unique scorable texts | rule: text hash | unique texts | 855,682 | 849,415 | 2,273,716 | 406,756 | 483,235 | 4,868,804 |
| **B. Prefilter: does it mention AI?** | | | | | | | | |
| Unique texts flagged | **model**: gradient boosting, 39 signals, threshold 0.17 | unique texts | 11,293 | 2,933 | 6,383 | 384 | 9,584 | 30,577 |
| as % of unique scorable texts | | | 1.3% | 0.3% | 0.3% | 0.1% | 2.0% | 0.6% |
| **C. Frames: what does it claim about AI?** | | | | | | | | |
| Unique texts classified | rule: every flagged text, one call each | unique texts | 11,294 | 2,933 | 6,384 | 384 | 9,584 | 30,579 |
| Unique texts with ≥1 frame | **model**: LLM judge, zero frames = discard | unique texts | 10,532 | 2,587 | 5,216 | 232 | 7,902 | 26,469 |
| as % of classified (the rest are prefilter false positives) | | | 93% | 88% | 82% | 60% | 82% | 87% |
| Unique frames | **model**: the judge decides how many | frames | 16,245 | 3,409 | 7,168 | 295 | 16,249 | 43,366 |
| Frames as instances (in documents) | rule: join by hash to every instance | instances | 17,758 | 4,590 | 7,284 | 313 | 16,270 | 46,215 |
| Documents with ≥1 frame | rule | documents | 1,681 | 1,431 | 1,090 | 206 | 2,709 | 7,117 |
| as % of the form's documents | | | 59% | 18% | 41% | 0.6% | 34% | 13% |
| **D. Activities: what does the firm say it does?** | | | | | | | | |
| Unique texts with a firm-subject behavioural frame | rule on the judge's labels: subject = firm and an adoption, capability, investment, talent or outcome concept | unique texts | 5,927 | 1,829 | 2,923 | 134 | 6,803 | 17,420 |
| as % of texts with a frame | | | 56% | 71% | 56% | 58% | 86% | 66% |
| Disclosed activities (unique) | **model**: second LLM pass with the filing firm in the prompt; rule: no evidence sentence, no activity | activities | 8,556 | 2,408 | 2,670 | 163 | 14,020 | 27,566 |
| activities per behavioural text | | | 1.4 | 1.3 | 0.9 | 1.2 | 2.1 | 1.6 |
| Activities as instances (in documents) | rule: join by hash | instances | 9,224 | 3,238 | 2,708 | 165 | 14,033 | 29,368 |
| Documents with ≥1 activity | rule | documents | 1,298 | 961 | 586 | 88 | 2,444 | 5,377 |
| Firms with ≥1 activity | rule: the document's ticker | firms | 409 | 159 | 269 | 55 | 383 | 471 of 510 |

: Inference funnel, S&P 500 constituents, U.S. filings and earnings calls. Each row states whether a deterministic rule or a model produces it; the texts labelled to train and evaluate the models are in @tbl-labelled. {#tbl-funnel}

The unit changes between blocks and is stated on every row: documents, paragraph instances, unique texts (a paragraph repeated across documents counts once), firms. Only three steps are model decisions, the prefilter flag, the frames and the activities; everything else, including every analysis in Chapters 3–7, is a deterministic rule on those labels. Three features of the funnel matter downstream. Deduplication: 8.35 million scorable paragraph instances are 4.87 million unique texts, and embedding, prefiltering and frame extraction happen once per unique text; the frames of each unique text are then joined back to every document in which it appears (43,366 unique frames become 46,215 in documents, a factor of 1.07, so repeated boilerplate weighs little). The judge discards 13% of what the prefilter flags: 26,469 of 30,579 flagged texts receive at least one frame; the rest are prefilter false positives that the extraction filters by returning zero frames. And the 8-K layer is almost pure noise: 34,416 documents, 206 with a frame. Earnings calls are the dense channel: a transcript has a tenth of the paragraphs of a 10-K, yet the 7,947 transcripts yield as many frames as the 2,873 annual reports, and half of all disclosed activities. Block D is the second extraction pass: of the 26,469 texts with a frame, 17,420 carry a behavioural frame about the firm itself and yield 27,566 distinct activities, 1.6 per text, 2.1 on calls. Per-form counts of unique texts add to more than the total because a text can appear in two forms.
...
The texts labelled to train and evaluate the models sit outside the inference funnel and are listed in @tbl-labelled. The prefilter is trained on an LLM-judged golden set of 9,900 unique 10-K and 10-Q texts with `GroupKFold` grouped by filing, so paragraphs from the same document never span train and test, tuned on 2,393 further proxy-statement and 8-K texts, and evaluated on a 1,500-text proxy and 8-K holdout that never enters any fit or threshold choice. The judge itself is checked against an independent model (Gemini) on 6,038 paired texts, with κ = 0.87 on whether a paragraph mentions AI and 0.86 on whether it is AI disclosure [@cohen1960]. Performance differs by venue, and the difference is carried as a caveat through the whole dissertation: on 800 judge-labelled earnings-call paragraphs, held out like the proxy and 8-K set, precision is 0.59 with recall 0.98; precision is 0.98 in 10-K and 10-Q text and 0.73 in proxy statements and 8-Ks; the weighted F1 is 0.96 in 10-K text against 0.82 in DEF 14A, 0.73 in calls and 0.65 in 8-K. The frame judge discards most prefilter false positives by returning zero frames, but the pending human sample is stratified by venue for this reason. Every comparison *between* venues therefore mixes a difference in discourse with a difference in measurement error; comparisons *within* a firm over time do not.

| Set | Forms | Unique texts | Positives (judge) | Labelled by | Used for | Result |
|---|---|---:|---:|---|---|---|
| Golden set | 10-K 6,991 · 10-Q 2,909 | 9,900 | 1,852 | qwen3.7-flash (all); Gemini on 6,038 | train the prefilter (GroupKFold by filing, stratum weights) and pick the threshold; strata: tech oversample 3,950, maximum variation 4,450, random 1,500 (validation) | weighted F1 0.96 in 10-K; precision 0.98 |
| Judge agreement | 10-K/10-Q | 6,038 paired | — | Gemini vs qwen | is the judge reproducible | κ 0.87 (mentions AI), 0.86 (AI disclosure), 0.81 (three-level relevance) |
| Tuning set, proxy and 8-K | DEF 14A 1,999 · 8-K 394 | 2,393 | 408 | qwen | train v2 outside the 10-K domain; disjoint from the holdout | — |
| Holdout, proxy and 8-K | DEF 14A 1,000 · 8-K 500 | 1,500 | 381 | qwen | out-of-domain evaluation; never in any fit; strata by lexical term strength | F1 0.82 (DEF 14A), 0.65 (8-K); precision 0.73, recall 0.92 |
| Holdout, earnings calls | calls | 800 | 376 (mentions AI) | qwen | out-of-domain evaluation; never in any fit; strata strong 400 / weak 200 / no term 200 | precision 0.59, recall 0.98, F1 0.73 weighted to the corpus |
| Anchors and lexicon | — | 17 positive and 10 negative anchors; 18 strong and 12 weak terms; ~60 entity names | — | hand-written | prefilter signals (cosine similarity, lexical gate) | — |
| Frames and activities | all | zero-shot with a schema; no training set | — | — | 30 random paragraphs × 3 prompt versions used to design the activity schema, not in any number | — |
| **Human validation (pending)** | all | 300 paragraphs with frames (434 frames) · 300 prefilter paragraphs by stratum · 120 paragraphs with activities (267 activities) | — | **human** | κ human–judge on promotional and temporal; prefilter precision and recall re-weighted by stratum, calls included; existence and per-field precision of the activities | pending |

: Texts labelled to train and evaluate the models. Every label that supports the analyses is an LLM label; the only agreement measured so far is between two LLMs. {#tbl-labelled}
...
## What firms say they are doing with AI

The segmentation says that Product Deployers devote more of their AI statements to deployment and outcomes; it does not say what they deploy. A second, narrower pass of the language model over the 17,420 paragraphs that carry a firm-subject behavioural frame turns each such statement into a *disclosed AI activity*: the firm performs an action (deploy, develop, integrate, buy or license, partner, invest in infrastructure, hire or train, pilot, scale, measure an outcome, govern, restrict) on an object (a copilot, a fraud model, a data centre) for a function (customer service, software development, underwriting), with a target, a stage, the *source* of the AI (the firm's own, a third party's, co-developed, acquired, open source), every company, product or model named with its role (the firm's own brand, an external provider or model, a partner, an acquired company, a customer, a distribution channel), and the strength of the evidence attached. The model is told which firm files the document, so that "AMD deploys Instinct accelerators" is the firm's own brand while "an insurer uses NVIDIA and Intel" names two external providers. Objects and functions are free text normalised afterwards into families. Each activity is counted once per firm. The pass yields 27,583 activities for 471 of the 510 firms; 51% appear only on earnings calls, so every share of firms below is also given for filings alone. These are activities the firm *says* it performs, with the supporting sentences as evidence.

Six in ten activities are deployments and seven in ten are declared deployed or scaled: the register is one of use, not exploration. The AI described is overwhelmingly the firm's own: 73% of activities state it as own and 8% as a third party's; 35% name one of the firm's own brands and 5% an external provider or model. But the typical activity is thin. The largest object family, present in 86% of firms, is "AI" with an empty noun ("AI capabilities"); a third of activities name no business function; more than half attach no product, metric or vendor. What firms say they do, concretely, is in @tbl-concrete.

| Action · object | % of firms | Literal objects, examples |
|---|---:|---|
| deploy · product or service with AI | 55.9 | AI engine, interconnect products, advertising tools |
| deploy · ML or predictive model | 45.7 | machine learning models, AI algorithms |
| deploy · analytics or data platform | 45.1 | AI platform, expert platform, AI-powered analytics |
| deploy · named product or platform | 34.1 | AIOps, AIP, Mist AI, Cerebrus |
| deploy · automation | 33.7 | automation, automation tools |
| hire or train · talent | 28.6 | AI talent, workforce |
| invest · compute and infrastructure | 25.9 | AI infrastructure, data centres, compute capacity |
| scale · product or service | 23.3 | data ecosystem, Firefly services |
| measure outcome · financial outcome | 19.6 | AI revenue, productivity gains |
| deploy · compute and infrastructure | 19.4 | AI PC, AI accelerators, GPU |
| deploy · copilot or assistant | 18.8 | Copilot, AI assistant, GitHub Copilot |
| deploy · vision, robotics or autonomy | 17.3 | autonomous driving system, Optimus robot |
| deploy · LLM or foundation model | 14.9 | large language models |
| develop · product or service | 14.7 | ad products, developer tools |
| deploy · search or recommendation | 14.3 | recommendation engine, discovery engine |
| deploy · AI agents | 13.3 | AI agents, agentic AI |
| deploy · security tool | 12.9 | ML-powered firewall, AI-driven security operations |
| integrate · product or service | 12.4 | AI into products and services |
| partner · (partner or ecosystem) | 11.8 | AI collaboration, cloud providers |
| deploy · chatbot | 11.8 | chatbot, conversational AI |

: The most common disclosed AI activities of the S&P 500: share of the 510 firms with at least one activity of each action · object type, filings and calls pooled. {#tbl-concrete}

With the function attached, the most common things an S&P 500 firm says it does with AI are: deploy an AI product or tool in operations (30% of firms), automate operations (27%), run predictive models (19%) or an analytics platform (18%) in operations, deploy an AI product for customer service (17.5%: virtual try-on, AI-powered experiences), put AI into the product itself (17%: full self-driving, Apple Intelligence, cockpits), a named product in operations (15%), compute for operations (12.5%), measure productivity (10%), marketing tools (10%), coding tools (9.6%), a copilot for customer service (9.2%), predictive models for marketing (9.2%) and for fraud and risk (8.8%), a recommendation engine for marketing (7.6%). On the source, 246 of 510 firms name an external provider, model or partner somewhere: OpenAI/Microsoft in 17.3% of firms (OpenAI, ChatGPT, Azure, GitHub Copilot), Google in 9.6%, NVIDIA in 9.0%, AWS in 6.9%, Meta in 3.5%, Anthropic and IBM in 2.4% each; another 41.8% name a provider outside those families (AMD, Palantir, ServiceNow, Hugging Face, Snowflake), which measures whom others name, not how much each vendor talks about itself.

The activity layer gives the segments tangible content (@tbl-activities-seg). Product Deployers *sell* their own AI with a brand: 97% name an own product or brand, 84% a named product or platform, 89% also name an external provider or partner, 62% invest in data centres and compute, 48% name a customer, two-thirds co-develop or acquire, and 80% of their activities state the AI as their own; the median Deployer has 112 activities. Governance Adopters automate operations (33%), deploy predictive models (46%) and analytics (42%) mostly inward (60% of their activities are internal), in fraud, risk and forecasting, with own but unbranded AI (20% of activities name a brand) and, in three activities out of ten, without saying where the AI comes from; they are also the segment of industrial vision and robotics (16%). Risk Listers have five activities at the median, one in ten none at all, and what little they describe is automation or an internal model; they have the largest shares of third-party AI (17% of activities) and of unstated source (34%), and only a third ever reach "scaled".

| | Product Deployers | Governance Adopters | Risk Listers |
|---|---:|---:|---:|
| Activities per firm (median) | **112** | 18 | 5 |
| deploy · AI product or service | 95 | 58 | 29 |
| deploy · named product or platform | **84** | 29 | 8 |
| deploy · analytics or data platform | 88 | 42 | 22 |
| deploy · ML or predictive model | 80 | 46 | 25 |
| deploy · automation | 65 | 33 | 16 |
| invest · compute and infrastructure | **62** | 23 | 7 |
| hire or train · talent | 61 | 26 | 13 |
| Own brand named (any) | **97** | 64 | 34 |
| External provider, model or partner named (any) | **89** | 46 | 27 |
| Named customer (any) | **48** | 10 | 5 |
| Co-developed or acquired AI (any) | **66** | 19 | 7 |
| Quantified outcome (any) | **94** | 65 | 37 |
| Share of activities aimed at customers / internal (mean) | 51 / 33 | 27 / 60 | 21 / 66 |
| Source of the AI: own / third party / unstated (% of activities) | 80 / 6 / 10 | 55 / 12 / 30 | 46 / 17 / 34 |
| Highest stage reached is "scaled" | 93 | 64 | 34 |

: Disclosed AI activities by segment: % of the segment's firms with at least one activity of each type, unless stated otherwise. {#tbl-activities-seg}

Firm inventories make the difference concrete. **Microsoft** (966 activities) deploys copilots (Copilot, GitHub Copilot, Security Copilot, Microsoft 365 Copilot; 168 activities, 86% with a named product) for customers and employees, invests in and scales AI infrastructure and compute capacity (142) with OpenAI, NVIDIA and AMD named as providers, offers Azure OpenAI to third parties, and hires AI talent; the AI it describes is its own in nearly every activity. **ServiceNow** (455) deploys AI agents and Now Assist for operations and customer service, with NVIDIA, Google, AWS and Celonis as partners. **HPE** (443) ships AI servers with NVIDIA chips to customers, under GreenLake, Private Cloud AI and Cray brands. **Etsy** (175) runs search algorithms and a recommendation engine (XWalk, Etsy Lens) for buyers and sellers, uses OpenAI and Google Cloud, and hires ML engineers. **Paychex** (153) deploys its Flex Intelligence Engine, labour-forecasting models, a Retention Insights tool and a Recruiting Copilot.

**JPMorgan** (31) deploys "AI/ML" in fraud, risk and marketing, inward, and in those statements names no product, provider or figure and does not say where the AI comes from. **State Street** (28) runs quantitative models in fraud and operations under State Street Alpha. **Cincinnati Financial** (13) builds underwriting and pricing models and pilots analytics with BCG and Deloitte. **Danaher** (23) names its instruments (DxI 9000, CytoFLEX, Mica) and CellXpress.ai. **Lowe's** (19) uses AI tools in operations with NVIDIA, OpenAI and Palantir named as providers, and Mylow, a virtual advisor for customers.

**Bank of America** (59) is the atypical Risk Lister: Erica as assistant, chatbot and product, a predictive language program for fraud, and explicit model-risk governance; the segment is defined by the share of risk language in the filing, not by the absence of activity. **Nike** (30) runs machine-learning and insights models in operations and marketing, and the SNKRS and NIKE apps. **Comerica** (2) has intelligent automation and mobile cheque scanning. **Howmet** (3) says "AI" in operations. **TransDigm** (1) automates processes.

## Voice against disclosed behaviour: two axes, nine cells

The grid is the direct form of the thesis question, does promotional voice exceed disclosed behavioural substance, with the two axes explicit instead of hidden inside a clustering. **Voice** is the share of a firm's AI statements in promotional or strategic-importance register (corpus mean 14.4%). **Disclosed behaviour** is the share that describes concrete conduct, a stage of use, a result or a capability (mean 44.9%). Both are shares over the firm's own frames, so neither rewards the firm that talks most; both are EB-shrunk; the 17 firms with no frames form their own cell. The correlation between the axes is 0.29: they are not the same axis, which is the condition for the grid to have content. Each axis is cut into terciles.
...
The disclosed activities say what lies behind the behaviour axis, and how concrete it is. For each firm, *behavioural concreteness* is the mean of five shares of its activities: with a named business function, deployed or scaled, with a named product or process, with a quantified outcome, with a named provider. A firm is *grounded* if it has at least one deployed or scaled activity with a named product or process. The behaviour axis and concreteness correlate at 0.24: they measure related but different things, how much of what is said is conduct, and how much of that conduct has a name, a stage, a figure or a provider.

| Corner | Firms | Activities (median) | Concreteness (mean) | Grounded | No activity at all |
|---|---:|---:|---:|---:|---:|
| Absolute decoupling (high voice, low behaviour) | 36 | 5.5 | 0.31 | **39%** | 8% |
| Quiet substance | 38 | 33 | 0.39 | 82% | 0% |
| Substantive and vocal | 90 | 88.5 | 0.36 | **91%** | 0% |
| Silent | 79 | 4 | 0.31 | 28% | 17% |
| Rest of the grid | 250 | 18 | 0.32 | 62% | 4% |

: Behavioural concreteness by corner of the grid. Grounded = at least one deployed or scaled activity with a named product or process. {#tbl-grounding}

Among the 170 high-voice firms there are two populations. 126 describe at least one deployed activity with a named product (median 58 activities; R&D 10.6% of sales, beta 0.99, 3.3 AI frames per 1,000 paragraphs: NVIDIA, Microsoft, Adobe, Alphabet, IBM, Salesforce, Palo Alto Networks, Intel). 44 describe none (median 5 activities; R&D 1.8%, beta 0.77, 0.36 frames per 1,000: Monolithic Power, Colgate, Electronic Arts, JPMorgan, Expeditors, Skyworks, Domino's, American Water). The second population is the one that matches the intuitive notion of washing: strategic claims with no identifiable activity behind them. The decoupled corner itself is not homogeneous: 14 of its 36 firms have a deployed activity with a name (Apple, UnitedHealth, Elevance, PayPal, Cigna, Discover, U.S. Bancorp), 22 do not (CSX, Textron, Southern, ONEOK, Hilton, General Mills, Kimberly-Clark, ConocoPhillips, Mondelez, KeyCorp). Concreteness is what splits it.
...
## Semantic style or identifiable activity?

The semantic block measures how AI is documented: realized, specific, promotional. The disclosed activities of Chapter 4 measure what the firm says it does. On the same main sample, six activity families per 1,000 paragraphs of the year's filings, with zeros (customer-facing deployment, internal deployment, proprietary AI, named provider, infrastructure investment, quantified outcome; log(1+x)), enter as an alternative block (M1 + activities) and as an additional block (M1 + semantics + activities). @tbl-decomp reports the increment over M1 and the p-value of the joint Wald test of each block with firm-clustered standard errors.

| Outcome | Semantics | Activities | Both | Activities given semantics | Semantics given activities | p act. | p act. given sem. | p sem. given act. |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Beta | +0.032 | +0.028 | **+0.051** | +0.019 | +0.023 | 0.012 | 0.055 | 0.021 |
| Idiosyncratic volatility | +0.025 | +0.030 | **+0.039** | +0.014 | +0.009 | <0.001 | 0.131 | 0.504 |
| Price / sales | +0.027 | +0.023 | **+0.044** | +0.017 | +0.022 | 0.034 | 0.029 | 0.008 |
| R&D / sales | +0.021 | +0.020 | **+0.037** | +0.015 | +0.017 | 0.002 | 0.008 | 0.025 |
| Revenue growth t+1 | +0.023 | +0.052 | **+0.063** | +0.040 | +0.011 | 0.042 | 0.035 | 0.235 |

: Decomposition of the content signal: ΔR² over M1 of the semantic block, the activity block, both, and each given the other; sector × year fixed effects, main sample. {#tbl-decomp}

Both blocks carry signal, and they carry different signal: together they raise the increment to four to six points, and each keeps half or more of its own increment once the other is in, with two symmetric exceptions. For idiosyncratic volatility, identifiable activity absorbs most of the style (semantics given activities +0.009, p = 0.50): idiosyncratic risk responds to what the firm does, not to how it tells it. For next-year revenue growth, the activities add twice what the style adds (+0.052 against +0.023) and the style nearly vanishes given the activities (p = 0.24): what anticipates growth is the concrete activity the firm describes. For beta and price-to-sales both hold. The reading for H1 is that specificity matters in addition to activity, not instead of it, and that where the outcome is about doing, idiosyncratic risk and growth, the activity leads.

## Reading for the thesis

The semantic parsing produces a coherent disclosure taxonomy (Chapter 4) and, in addition, incremental economic signal: two to four points of partial R² over fundamentals and volume on idiosyncratic risk, valuation, R&D and growth, concentrated in the specificity of the claims, and surviving adjusted R², a joint cluster-robust Wald test and a permutation within sector and year. Decomposed, the signal comes both from identifiable disclosed activity and from the style in which it is documented; for idiosyncratic risk and growth, from the activity. It is a signal of *type of firm* more than of *change within the firm*. The result would have supported the thesis with any sign: had the increment been zero, the conclusion would have been "taxonomy yes, economic signal no". It came out positive and small, and that is what is written.
...
Does the call only change the tone, or also what the firm says it is doing? The disclosed activities answer that within the same firm and fiscal year, on the 459 cells of 183 firms with at least three activities in each venue: the share of the venue's activities in each family, call minus filing (@tbl-activity-gap, @fig-activity-gap).

| Activity family | Call | Filing | Gap (pp) | t |
|---|---:|---:|---:|---:|
| Customer-facing deployment | 43.5 | 41.3 | +2.2 | 1.8 |
| Internal deployment | 24.4 | 23.8 | +0.6 | 0.5 |
| Own AI | 71.8 | 70.4 | +1.5 | 1.0 |
| Infrastructure investment | 5.9 | 7.9 | −2.0 | −3.0 |
| **Quantified outcome** | **17.1** | **6.2** | **+11.0** | 13.3 |
| **Named business function** | **76.5** | **64.8** | **+11.7** | 8.6 |
| Named external provider, model or partner | 8.1 | 2.8 | +5.3 | 8.0 |
| Named product or process | 30.3 | 26.5 | +3.8 | 3.0 |
| Piloting or exploring | 5.3 | 3.2 | +2.1 | 3.9 |
| Talent or training | 1.1 | 2.7 | −1.7 | −4.9 |
| Governance or restriction | 0.3 | 2.2 | −1.9 | −6.1 |

: What the same firm says it does with AI on the call and in its filings, same fiscal year: share of the venue's activities in each family, 459 cells, 183 firms. {#tbl-activity-gap}

![Share of activities on the call minus in filings, by family and fiscal year, percentage points of the venue's activities.](figures/fig_brecha_actividades.png){#fig-activity-gap}

The mix of activities is the same in both venues: the shares of customer-facing deployment, internal deployment and own AI do not differ (gaps of +0.6 to +2.2 points, none distinguishable from zero at 5%). What changes is the evidence and the detail. On the call the same activity comes with a business function (+11.7 points), a quantified result (+11.0: the filing almost never puts the figure), a named external provider or partner (+5.3, nearly three times the filing's share) and a named product (+3.8). The filing keeps infrastructure, talent and governance. Read in activities, the cross-venue gap is not "different things to different audiences" but "the same things, with numbers and names for the analyst and without them in what is signed". It is stable by year: the quantified-outcome gap runs between +9.0 and +11.8 points from 2021 to 2025, the named-function gap grows from +5.0 to +16.7, and the named-product gap from −1.4 to +8.8.
...
Both venues pass through the same prefilter and judge with different measurement error (precision 0.98 in 10-K and 10-Q, 0.73 in proxy and 8-K, 0.59 in calls, with recall above 0.92 in all); part of the *level* of the gap may be differential error, but the within-firm event study does not suffer from it. Intensity per paragraph depends on genre: a call has about 70 paragraphs and a 10-K about 500, so the gap in frames per 1,000 paragraphs is also a gap in genre density, which is why the share-based check is reported. Transcripts cover about 3.2 of 4 calls per firm-year and end in mid-2025. The promotional flag is an LLM label pending human validation. Designs the gap does not support are stated rather than attempted: a difference-in-differences with treatment defined by the prior gap is mean reversion; a synthetic control needs a longer pre-period than four annual points.
...
The correlations between them are low: Spearman −0.07 between the cross-venue gap and the conditional excess; seven of the eight firms flagged for excess fall in the "substantive and vocal" corner of the grid, not in the decoupled one. **These are three phenomena: talking without doing, talking more than what is done justifies, and telling different things to different audiences.** Read through the disclosed activities, the first two are two kinds of washing-consistent disclosure: *unsupported promotion*, high voice with no identifiable deployed activity behind it (the 44 firms of Chapter 4, with R&D at 1.8% of sales and beta 0.77), and *promotional excess*, abundant concrete activity and still a promotional rate far above what that activity predicts (the eight firms of this chapter). The cross-venue gap is a third dimension, of consistency across venues rather than of level. The third coincides with the mechanism the SEC named in the Welltower correspondence; the greenwashing literature usually measures the first; the second is the one a per-firm statistical score can test. The first and third were built in Chapters 4 and 6; what follows is the second.
...
The disclosed activities separate promotional intensity from behavioural grounding. The eight flagged firms have between 112 and 720 activities and between 21 and 275 deployed activities with a named product or process (Palo Alto Networks 275, Alphabet 238, Salesforce 148, CrowdStrike 121, Cadence 109); their behavioural concreteness (0.35–0.49) sits in the upper half of the corpus. Promotional excess is uncorrelated with concreteness (Spearman −0.01, n = 438) and with the number of grounded activities (−0.03). In the top quintile of promotional rate, 78 of 90 firms have a deployed activity with a named product (median 103 activities); the twelve that do not (JPMorgan, Gilead, Cummins, Invesco, Globe Life, PVH, PACCAR) have five activities at the median. Promoting and describing concrete activity go together; the excess the test detects is promotion on top of a lot of conduct, not promotion without conduct. Promotion without identifiable activity exists, but it is another population, the 44 high-voice firms without a grounded activity of Chapter 4, which this test does not flag because they have too few frames.
...
Third, and centrally, the content of AI disclosure carries information beyond fundamentals and volume: two to four points of partial R² on beta, idiosyncratic volatility, price-to-sales, R&D and next-year revenue growth, confirmed by bootstrap, adjusted R², a joint cluster-robust Wald test and a permutation test within sector and year. The dimension that carries it is specificity, and decomposed against the concrete activities firms say they perform, the signal comes from both the identifiable activity and the way it is documented. Promotional tone carries none. It is a signal of type of firm more than of change within the firm.
