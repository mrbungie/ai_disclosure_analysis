...
#| fig-cap: "[Open-weight vocabulary in earnings calls, monthly, with the fitted trend before and after January 2025.]{custom-style=\"CaptionText\"}"
#| echo: false
#| fig-width: 6.6
#| fig-height: 3.2
import pandas as pd
...
fig, ax = plt.subplots()
ax.plot(_ds["x"], _ds["rate_per_1k"], "o", ms=3.4, color=MIB["ink_light"], label="Monthly rate")
for side in (0, 1):
    seg = _ds[_ds["post"] == side]
    ax.plot(seg["x"], seg["fitted"], "-", lw=2.0, color=MIB["ink"] if side == 0 else MIB["terracotta"],
            label="Fitted trend, before" if side == 0 else "Fitted trend, after")
_break = pd.Timestamp("2025-01-01")
ax.axvline(_break, color=MIB["ink_800"], ls=":", lw=1.2)
ax.annotate("DeepSeek-R1", xy=(_break, ax.get_ylim()[1] * 0.94), xytext=(6, 0),
            textcoords="offset points", fontsize=8, color=MIB["ink_800"])
ax.set_ylabel("Open-weight terms per 1,000 words")
ax.set_xlabel("")
...
Open-weight vocabulary in earnings calls steps up at the turn of 2025 and decays afterwards (@fig-deepseek-break). Over `{python} str(_dsj["months"])` months of calls, the estimated step is `{python} f"{_dsj['step']:+.4f}"` terms per 1,000 words (95% CI `{python} f"[{_dsj['step_ci_low']:+.4f}, {_dsj['step_ci_high']:+.4f}]"`, $p$ `{python} pstr(_dsj["p_step"])`), against a monthly mean of `{python} f"{_dsj['mean_before']:.4f}"` before and `{python} f"{_dsj['mean_after']:.4f}"` after, with the post-event slope turning negative (`{python} f"{_dsj['slope_change']:+.5f}"`, $p$ `{python} pstr(_dsj["p_slope_change"])`) -- a spike that fades rather than a new level.
