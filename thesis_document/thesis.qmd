...
The eight regressions share one specification and are reported together in @tbl-market-results. Read down the disclosure columns rather than across the outcomes: the same measure behaves consistently, and it is the measure rather than the outcome that carries the finding.
...
**Posture composition separates firms where the outcome is a market price.** The joint test of the composition rejects for market beta ($p$ `{python} f"{_joint('beta_post_63'):.3f}"`), for size ($p$ `{python} f"{_joint('log_market_cap_post'):.3f}"`) and for down-to-up volatility ($p$ `{python} f"{_joint('duvol_post'):.3f}"`), and not for any accounting outcome. The separation runs through the Governance vertex: relative to a Defensive posture, weight in Governance goes with lower systematic risk ($\beta =$ `{python} b_of("beta_post_63", "w_gov")`, $p$ `{python} p_of("beta_post_63", "w_gov")`), larger size ($\beta =$ `{python} b_of("log_market_cap_post", "w_gov")`, $p$ `{python} p_of("log_market_cap_post", "w_gov")`) and less downside volatility ($\beta =$ `{python} b_of("duvol_post", "w_gov")`, $p$ `{python} p_of("duvol_post", "w_gov")`). It also separates from the Vocal vertex on beta (@tbl-market-contrasts), so this is not merely a contrast against the residual posture.
...
Across the `{python} str(mkt_m)` disclosure coefficients reported here, `{python} str(mkt_n_surv)` survive a Benjamini-Hochberg correction at five percent, against a rank-one threshold of `{python} mkt_crit1`: intensity on size, on down-to-up volatility and on negative skewness, historical decoupling on beta, and the disclosure indicator on both crash-risk measures. Every association involving posture composition or single-call decoupling falls outside.

```{python}
#| label: tbl-market-results
#| tbl-cap: "[Every estimated term of the eight regressions. Standardized coefficients with p-values in parentheses, starred at *** p<0.01, ** p<0.05, * p<0.10 and set bold below 5%; posture weights stay on their 0-1 scale, so ten points of reallocated weight is a tenth of the coefficient.]{custom-style=\"CaptionText\"}"
#| echo: false
from IPython.display import Markdown

# Rows are the regressors and controls as estimated. The posture terms are
# written as the contrasts they are: three read straight off the fit, the
# fourth is a difference of two coefficients and carries its own test built
# from their covariance. Fixed effects are absorbed, so they are marked
# present rather than enumerated.
_ROWS = [
    ("v", "w_call",              r"$W_{\text{call}}$"),
    ("v", "w_expanding",         r"$W_{\text{exp}}$"),
    ("v", "intensity_expanding", "Cumulative disclosure intensity"),
    ("c", "Defensive vs. no posture",    r"Defensive vs. no posture ($\beta_{\text{has}}$)"),
    ("c", "Vocal vs. Defensive",         r"Vocal vs. Defensive ($\beta_{\text{voc}}$)"),
    ("c", "Governance vs. Defensive",    r"Governance vs. Defensive ($\beta_{\text{gov}}$)"),
    ("c", "Governance vs. Vocal",        r"Governance vs. Vocal ($\beta_{\text{gov}} - \beta_{\text{voc}}$)"),
    ("c", "Composition (joint)",         "Composition, joint test ($p$)"),
    ("l", None,                  r"$Y_{t-1}$"),
    ("v", "log_market_cap",      "Log market capitalization"),
    ("v", "return60",            "Return, 60 days pre-call"),
    ("v", "roa",                 "Return on assets"),
]
_PANELS = [("beta_post_63", "log_market_cap_post", "ncskew_post", "duvol_post"),
...
def _show(b, p):
    ...
    if pd.isna(b):
        return f"**{p:.3f}**{_stars(p)}" if p < 0.05 else f"{p:.3f}{_stars(p)}"
    body = f"{b:+.3f}{_stars(p)} ({p:.3f})"
    return f"**{body}**" if p < 0.05 else body

def _cell(kind, key, target):
    if kind == "c":
        row = _con[(_con["target"] == target) & (_con["contrast"] == key)]
        if not len(row):
            return "--"
        return _show(float(row["beta"].iloc[0]), float(row["p"].iloc[0]))
    sub = _bat[_bat["target"] == target]
    name = sub["pre_control"].iloc[0] if kind == "l" else key
    row = sub[sub["variable"] == name]
    return "--" if not len(row) else _show(float(row["beta_std"].iloc[0]), float(row["p"].iloc[0]))

def _meta(target, column, fmt):
    return fmt(_bat[_bat["target"] == target][column].iloc[0])

_lines = []
for panel, targets in zip("AB", _PANELS):
    heads = [_bat[_bat["target"] == t]["label_outcome"].iloc[0] for t in targets]
    _lines += [f"| **Panel {panel}** | " + " | ".join(heads) + " |",
               "|:---|" + "---:|" * len(targets)]
    for kind, key, label in _ROWS:
        _lines.append(f"| {label} | " + " | ".join(_cell(kind, key, t) for t in targets) + " |")
    for label, value in [("Sector fixed effects", lambda t: "OK"),
                         ("Year fixed effects", lambda t: "OK"),
                         ("SE clustered by firm", lambda t: "OK"),
                         ("Calls", lambda t: _meta(t, "n_calls", lambda v: f"{int(v):,}")),
                         ("Firms", lambda t: _meta(t, "n_firms", lambda v: str(int(v)))),
                         ("$R^2$", lambda t: _meta(t, "r2", lambda v: f"{float(v):.3f}"))]:
        _lines.append(f"| {label} | " + " | ".join(value(t) for t in targets) + " |")
    _lines.append("")
Markdown("\n".join(_lines))
```

The weights are reallocations, so their coefficients answer a question of composition. @tbl-market-contrasts reports all four comparisons the three postures support, for the two outcomes where the disclosure block is otherwise informative. Governance against Vocal is the difference of two estimated coefficients and requires the covariance between them; it cannot be read off the two columns above.

```{python}
#| label: tbl-market-contrasts
#| tbl-cap: "[Posture contrasts for the two crash-risk outcomes. Each is a reallocation of archetype weight holding the remaining vertex constant, with p-values in parentheses.]{custom-style=\"CaptionText\"}"
#| echo: false
_CONTRASTS = ["Defensive vs. no posture", "Vocal vs. Defensive",
              "Governance vs. Defensive", "Governance vs. Vocal", "Composition (joint)"]
_lines = ["| Contrast | NCSKEW | DUVOL |", "|:---|---:|---:|"]
for name in _CONTRASTS:
    cells = []
    for target in ("ncskew_post", "duvol_post"):
        b, p = ctr(target, name)
        cells.append(_show(b, p))
