...
The main hypothesis splits into two claims tested separately: that posture and grounded substance carry incremental information about firm operating characteristics (H-operational), and that they carry incremental information about market risk (H-market). Across `{python} hd_paragraphs_m` million filing and earnings-call paragraphs from `{python} f"{hd_n_docs:,}"` SEC filings and transcripts, the study identifies `{python} f"{hd_n_activities:,}"` disclosed AI activities across `{python} hd_n_activity_firms` firms, and H-operational holds: three stable communication archetypes emerge, and posture predicts disclosed activity beyond narrative volume alone. Vocal Substantives pair frequent discussion with broad, mature disclosed activity portfolios, while Governance-Led and Defensive Disclosers rely more on oversight or risk language and disclose less activity at comparable narrative volume. Disclosed activity is mainly applied rather than foundational, embedding external AI tools in products, customer service, and internal workflows. The generative-AI boom multiplied the number of disclosed activities but not their average evidentiary grounding -- a fact about what firms chose to document under rising narrative pressure, not about whether their underlying AI programs improved or worsened.

Credibility also depends on venue. Within the same firm-year, calls and filings describe a similar activity mix, yet calls contain `{python} round(hd_promo_ratio)`× more promotional and `{python} round(hd_quant_ratio)`× more quantified claims per thousand words (pooled per-1,000-word rates without firm-year fixed effects, part of which reflects the longer, boilerplate-heavy denominator of statutory filings), and quantified outcomes are `{python} f"{hd_gap_quant_pp:.0f}"` percentage points less common in the matching filings. The narrative reaches the binding venue; much of the evidence does not. This decoupling is not evidence of a discrete "washing" style adopted by some firms -- it is a continuous, persistent firm-year attribute that serves as a due-diligence signal, not proof of deception. The single direct SEC inquiry observed, involving Welltower, is consistent with regulatory questioning narrowing this gap, though one case cannot establish a general regulatory effect.

H-market receives little support. Historical AI disclosure intensity, rather than disclosed substance or call-specific surprises, is the signal associated with subsequent systematic risk and several firm fundamentals, and these are non-causal, cross-sectional associations: firms with persistently higher narrative volume already show higher systematic risk before any given call, so the call itself does not appear to move it. That capital markets do not reward the more verifiable evidence behind a firm's AI claims is itself an informative result, not a null to explain away. The one exception is downside tail risk: the Governance-Led archetype is associated with lower crash-risk exposure ($\beta = -0.150, p = 0.0016$), the sole archetype effect that survives multiplicity correction across the outcomes tested. The framework therefore does not label firms as fraudulent or technologically capable. It gives managers, boards, regulators, and investors a disciplined, three-step way to move beyond AI mention counts: identify a firm's disclosure posture, test whether that posture is backed by grounded execution, and check whether the same claims hold from earnings calls to statutory filings.
...
* **Signalling and cheap talk.** Corporate claims gain credibility when they contain details that are costly to fake or easy to verify [@spence1973, pp. 363--64; @crawford1982, pp. 1431--32; @dye1985, pp. 123--24]. When talk is cheap and legal liability is low, economic theory predicts ungrounded promotional claims. Assessing credibility requires separating generic rhetoric from hard-to-fake evidence; this thesis operationalizes such evidence as named systems, deployment stages, quantified metrics, and identified technology partners, building on evidence that optimistic rhetoric unsupported by facts raises shareholder litigation risk [@rogers2011, pp. 2155--56].
...
* **Decoupling and symbolic management.** Organizational sociology distinguishes formal statements from day-to-day operations [@meyer1977, pp. 340--41; @westphal1994, pp. 367--69; @crilly2012, pp. 1429--31; @bromley2012]. @seele2022 extend this decoupling logic to algorithmic communication, conceptualizing machinewashing and identifying organizational decoupling as a relevant research perspective for it. This thesis examines a narrower, observable version of that gap: the distance between narrative volume and grounded activities disclosed by the same firm, a textual measure that does not by itself establish a discrepancy between disclosure and actual implementation.
* **Regulatory enforcement and credibility boundaries.** Regulators can alter the cost of corporate talk [@gensler2023, pp. 4--5; @sec2024pressrelease]. Formal SEC filings and voluntary earnings calls differ in preparation and legal exposure: filings are attorney-reviewed statutory documents, while calls are prepared presentations followed by open, less formal analyst discussion [@matsumoto2011, pp. 1383--84; @frankel1999, pp. 133--34]. This asymmetry provides a setting to test whether external scrutiny disciplines cheap talk.
...
4. **Capital-market relevance:** Prior studies examine the informativeness of AI disclosures and their relationship with investment and market outcomes. This thesis asks whether activity-level grounding and internal narrative--substance decoupling add information beyond disclosure intensity and communication posture in the outcomes and horizons examined here.
...
- **RQ2: Strategic heterogeneity and economic coherence.** *Which recurring communication archetypes set firms apart, and does that posture cohere with disclosed operational activity and economically relevant firm outcomes?* Derived from configuration theory and signalling, this question tests whether distinct disclosure postures reflect real economic endowments rather than random noise.
...
Each pipeline stage was rigorously benchmarked against holdout evaluations and blind human audits (@tbl-evaluation-datasets, Appendix B). Cross-model comparisons confirm strong inter-rater reliability between independent foundation models (Gemini and Qwen agreement $\kappa = 0.81\text{--}0.87$, evaluating inter-model consistency across semantic dimensions), but that comparison is between two non-production models and does not by itself validate the production judge, `gpt-5.6-luna`. That validation instead comes from blind human audits directly on the production judge's output: manual review establishes 91.2% precision on GBDT prefiltering and 89.1%–95.8% field-level accuracy across its extracted activity parameters.
...
2. **Failed falsification at $k = 4$:** Partition stability breaks down globally at $k = 4$. The fourth vertex isolates ungrounded promotional rhetoric without technical specificity, anchored by only a handful of firms -- too few, and too unstable under resampling, to support a discrete fourth archetype.
...
Following @tukey1977 and @gelman2014, these market evaluations are framed as exploratory diagnostics rather than confirmatory trials. To discipline the analysis against the garden of forking paths inherent in multi-outcome observational testing, we pre-specify theoretical channels, apply Benjamini–Hochberg False Discovery Rate corrections, and employ unified martingale-conditioned ANCOVA specifications. The findings distinguish persistent communication posture from quarterly surprises, testing whether markets reward substantive execution or promotional narrative.

## Theoretical channels and hypotheses

We test three distinct economic channels connecting AI disclosure to market outcomes (@tbl-market-channels):
...
The unit of observation is a firm-date earnings call event ($i,t$) from 2021 to 2025 (transcripts on the same date are aggregated; zero-mention calls remain as valid zeros). Because asset prices, beta, and fundamentals follow persistent martingale processes ($\mathbb{E}[Y^{\text{post}}_{i,t} \mid \mathcal{F}_{t^{-}}] \approx Y^{\text{pre}}_{i,t}$), static regressions conflate unobserved firm heterogeneity with call impact. We use a unified Analysis of Covariance (ANCOVA) conditioning directly on the outcome's own predetermined lag ($Y^{\text{pre}}_{i,t}$ at $t-1$):
...
We test whether disclosure intensity acts as thematic factor exposure, shifting systematic risk ($\beta$) and valuation multiples. Regressors are $\text{HistD}$ and $\text{SurpriseD}$, controlling for $\text{HistS}$ and $\text{SurpriseS}$. @fig-call-beta-baseline reports the baseline beta estimates.
...
Across `{python} f"{int(_formal['n_calls']):,}"` calls and `{python} int(_baseline_sample['n_firms'])` firms, historical disclosure intensity is positively associated with post-call market beta ($\beta = +0.079, p = 0.006$), whereas single-call surprises and substantive operational activity are statistically indistinguishable from zero (joint disclosure $F = 5.55, p = 0.004$; joint substance $F = 0.35, p = 0.703$; @fig-call-beta-baseline). In leave-one-year-out re-estimations, this historical disclosure effect remains significant across all seven single-year exclusions ($p \le 0.023$), indicating that the association reflects persistent posture rather than any single call or reporting year.

We extend this to valuation and revenue growth in @fig-economic-coefficients-c1.
...
SERIES_C1 = [
    ("hist_disclosure", "HistDisclosure", "#059669"),
    ("surprise_disclosure", "SurpriseDisclosure", "#0EA5E9"),
    ("hist_substance", "HistSubstance (control)", "#7C3AED"),
    ("surprise_substance", "SurpriseSubstance (control)", "#F59E0B"),
]
...
ax.axvline(0, color="#334155", lw=1, linestyle="--")
ax.set_yticks(y0)
...
The multi-target ANCOVA specifications displayed in @fig-economic-coefficients-c1 and @fig-economic-coefficients-c2 simultaneously include strategic archetype indicator dummies ($\mathbf{1}_{\{\text{GovLed}\}}, \mathbf{1}_{\{\text{Vocal}\}}$). Because these archetype indicators absorb between-archetype differences in communication posture, the continuous $\text{HistDisclosure}$ coefficient on post-call market beta attenuates to $\beta = +0.051$ ($p = 0.140$), compared to $\beta = 0.076$ ($p = 0.0097$) when the same baseline specification is re-estimated without archetype indicators on the archetype sample ($N = 4,102$; the full-sample baseline of @fig-call-beta-baseline gives $0.079, p = 0.006$). Valuation and revenue growth, by contrast, remain robustly positive even conditional on archetype controls.
...
Robustness tests reported in @tbl-call-beta-paper (Appendix E) confirm that the historical disclosure effect is not an artifact of model specification. The association survives point-in-time accounting controls (ROA and leverage; $\beta =$ `{python} f"{rob_acct['beta_std']:.3f}"`, $p =$ `{python} f"{rob_acct['p']:.3f}"`), a full-year post-call window (+252 days; $\beta =$ `{python} f"{rob_win252['beta_std']:.3f}"`, $p =$ `{python} f"{rob_win252['p']:.3f}"`), and the Fama–French 3-factor risk benchmark ($\beta =$ `{python} f"{rob_ff3['beta_std']:.3f}"`, $p =$ `{python} f"{rob_ff3['p']:.3f}"`). Under firm fixed effects, historical disclosure attenuates ($\beta =$ `{python} f"{rob_fe['beta_std']:.3f}"`, $p =$ `{python} f"{rob_fe['p']:.3f}"`), while first differences ($\Delta\beta$) yield an attenuated estimate with wide confidence intervals ($\beta =$ `{python} f"{rob_delta['beta_std']:.3f}"`, $p =$ `{python} f"{rob_delta['p']:.3f}"`). Combined with pre-call placebo tests ($\beta^{\text{pre}} \sim D$ yields $\beta = +$`{python} f"{rob_placebo_d['beta_std']:.3f}"`, $p < 0.001$, while substance is null), this pattern indicates cross-sectional sorting into thematic risk: firms establishing high narrative visibility carry higher systematic beta, but quarterly changes in disclosure volume do not move beta over time.

## Channel 2 — Disclosed substance and operating fundamentals (H6b)

We test the costly signaling hypothesis ($H_{6b}$): whether disclosed substance ($S$) conveys an incremental credible signal rewarded in operating fundamentals and reduced market risk. Regressors are $\text{HistS}$ and $\text{SurpriseS}$, controlling for $\text{HistD}$, and including strategic archetype dummies ($\mathbf{1}_{\{\text{GovLed}\}}, \mathbf{1}_{\{\text{Vocal}\}}$ vs. Defensive Disclosers).
...
SERIES_C2 = [
    ("hist_substance", "HistSubstance", "#7C3AED"),
    ("surprise_substance", "SurpriseSubstance", "#F59E0B"),
    ("hist_disclosure", "HistDisclosure (control)", "#059669"),
    ("dum_gov", "Governance-Led (vs Def.)", "#EC4899"),
    ("dum_voc", "Vocal Substantives (vs Def.)", "#6366F1"),
]
...
We evaluate Channel 3 ($H_{6c}$): whether narrative decoupling ($W = \text{pctrank}(D) - \text{pctrank}(S)$) represents cheap talk and opacity. Adding firm-year $W$ to the baseline beta specification (on `{python} f"{hd_inc_n:,}"` calls) yields $\Delta R^2 = +0.0002$ with a Wald test of $F = 0.69$ ($p = 0.406$), failing to reject the null of zero incremental explanatory power. Decoupling is not a priced systematic risk factor: theoretical models of cheap talk and information opacity [@crawford1982; @jin2006] predict that narrative decoupling should not alter market-wide covariance ($\beta$), but should instead manifest in firm-specific tail risk.

### Stock price crash risk (NCSKEW)

We evaluate the tail risk channel by estimating Negative Coefficient of Skewness ($NCSKEW$) and Down-to-Up Volatility ($DUVOL$) over $[+21, +126]$ trading days post-call (@fig-call-crash-risk). In line with literature documenting that qualitative corporate disclosures in earnings conference calls convey price-relevant information that constrains stock price crash risk [@fu2021], this horizon captures whether AI narrative commitments alter the distribution of extreme negative returns once quarterly communications are digested.
...
colors = ["#7C3AED" if v == "dum_gov" else "#64748B" for v in order]
...
::: {custom-style="FigureNote"}
*Notes:* Standardized coefficients and 95% confidence intervals clustered by firm, with industry-by-year fixed effects ($\text{SIC2} \times \text{Year}$) and lagged pre-call crash risk ($N = 8,199$ calls across 310 firms, total $R^2 = 0.312$, partial $R^2 (\text{archetype block}) = 0.0031$). The Governance-Led indicator ($\beta = -0.150, p = 0.0016$) is highlighted in purple; it is the sole estimate surviving Benjamini–Hochberg FDR multiplicity correction across the 18-test family ($p \le 0.05/18 \approx 0.0028$).
:::

The crash risk estimates in @fig-call-crash-risk indicate two main findings:

First, continuous decoupling shows no short-run association with stock price crash risk over a 6-month horizon ($\beta = -0.005, p = 0.814$), running counter to an immediate short-run interpretation of $H_{6c}$; evaluating whether bad news accumulates over longer multi-year horizons remains an empirical extension.

Second, the indicator for *Governance-Led Disclosers* exhibits a statistically significant negative association with downside crash risk relative to Defensive Disclosers ($\beta = -0.150, p = 0.0016$ on $NCSKEW$; $\beta = -0.153, p = 0.0406$ on $DUVOL$). Across the full 18-test family of archetype hypotheses, this $NCSKEW$ coefficient is the sole estimate to clear Benjamini–Hochberg [@benjamini1995] FDR multiplicity correction ($p = 0.0016 \le 0.05 \times 1/18 \approx 0.00278$). This aligns directly with theoretical accounts where governance and structured risk disclosures mitigate bad-news withholding [@hutton2009; @kim2011; @au2023]. In particular, @au2023 show that structured risk factor disclosures compress crash risk ($NCSKEW$ and $DUVOL$) primarily when backed by robust governance mechanisms, separating disciplined oversight disclosures from unconstrained voluntary boilerplate. Three robustness checks clarify the identification behind this result:

1. *Martingale structure:* The pre-call $NCSKEW$ control absorbs persistent level differences across sectors and firms; an incremental post-call divergence is not absorbed by static sorting. Under this specification, the Governance-Led coefficient reflects the incremental post-call association over the lagged baseline.

2. *Excluding regulated sectors:* Excluding Financial Services (SIC 60–67), Utilities (SIC 49), and Healthcare & Pharma (SIC 28, 38, 80), the Governance-Led coefficient remains negative and significant ($\beta = -0.176, p = 0.021$). Non-regulated exemplars such as Alaska Air (`ALK`), Delta (`DAL`), and RTX (`RTX`) indicate that the tail-risk association is not an artifact of utility or banking regulation.

3. *Posture switchers:* Among firms transitioning into or out of Governance-Led over time, the coefficient remains similar in sign and magnitude ($\beta = -0.161$), with wider confidence intervals reflecting the smaller sample ($p = 0.063$, 95% CI $[-0.331, +0.009]$). Because the model controls for $\text{SIC2} \times \text{Year}$ fixed effects rather than firm fixed effects, this reflects variation across switchers and non-switchers within industry-years, providing consistent exploratory support.

Taken together, these checks indicate that the negative association between Governance-Led disclosure and crash risk is a consistent exploratory pattern across specifications: it survives multiplicity control across the 18-test family, persists across sector exclusions, and aligns with theoretical mechanisms where credible risk disclosures and internal controls mitigate bad-news hoarding [@hutton2009; @kim2011; @au2023]. This finding also qualifies recent empirical evidence by @yang2026, who finds that enterprise AI adoption reduces crash risk ($NCSKEW$ and $DUVOL$) in Chinese listed firms through improved internal control quality and decision efficiency. In our S&P 500 setting, simply escalating AI mention volume or narrative intensity does not compress tail risk—continuous disclosure and substance surprises remain statistically indistinguishable from zero—but the Governance-Led posture is the disclosure configuration that exhibits the robust negative association with tail risk.
...
**Microsoft (MSFT):** Microsoft demonstrates that heavy narrative volume does not imply decoupling. Its disclosure intensity is exceptional ($D = 1.84$ frames/1k, 100th percentile), but it is backed by one of the largest disclosed activity inventories in the corpus and strong operational grounding ($S = 2.43$, 99th percentile). Talk and substance align ($W = +0.004$).

**JPMorgan Chase (JPM) vs. Goldman Sachs (GS):** Two financial institutions with similar moderate disclosure volume illustrate how communication posture differentiates firms without creating decoupling. In 2024, both report modest intensity (54th and 59th percentiles). While JPMorgan anchors communication in board oversight and internal workflow controls ($G = 0.299$), Goldman Sachs concentrates on Item 1A risk warnings paired with technical specificity across operational mentions ($G = 0.439$). Both exhibit negative decoupling ($W = -0.065$ and $-0.068$), showing that neither allows narrative to outrun substantive disclosure.

**Apple (AAPL):** Apple exemplifies the Governance-Led posture in practice. Its communication stresses proprietary on-device processing, privacy principles, and board oversight in proxy statements (DEF 14A) rather than commercial product claims in 10-K filings. Because governance narrative presence outpaces operational disclosures in statutory filings, it carries a moderate positive decoupling score in 2024 ($W = +0.176$), which compresses toward zero as operational disclosures catch up in subsequent cycles.

**Welltower (WELL, SEC comment letter):** Welltower illustrates the regulatory consequence of cross-venue divergence ($W = +0.229$): oral conference-call claims outpaced statutory 10-K disclosures, prompting a formal SEC comment letter (examined in detail in Chapter 5 and Appendix F).

## Portfolio-scale application: screening the S&P 500

### Step 2 triage: the disclosure intensity/substance heuristic

With posture established in Step 1 (Chapter 3), Step 2 asks whether that posture is backed by grounded execution. The framework organizes this through a 2×2 decision heuristic built on Disclosure Intensity ($D$) and Disclosed Substance ($S$) (@tbl-screening-matrix): $D$ and $S$ jointly determine which quadrant, and therefore which action, a firm falls into. The decoupling score $W$ does not define the quadrant and does not serve as a fraud classifier ($W \neq \text{Legal Wrongdoing}$); it is a continuous ranking used to prioritize firms *within* a quadrant, not a label attached to it. This structure matches how @simonian2025, in a CFA Institute report, frames due diligence on AI claims: stakeholders are asked to examine the methods, data, performance evidence, and organizational expertise behind them. The screening matrix below covers only the first of those steps, organizing reported activities, deployment stages, quantified outcomes, and named providers into a structured initial read of the public disclosures -- flagging which claims warrant the closer follow-up that Simonian's fuller questionnaire describes, since assessing model performance, data quality, and actual implementation requires evidence beyond what is analyzed here.
...
**Residual decoupling index ($W^*$, Jones-model analogue).** To verify that non-parametric rank subtraction does not introduce artifacts relative to parametric residualization, we estimate an earnings-management style regression [@jones1991] predicting substance ranks from disclosure ranks while controlling for firm scale and two-digit SIC industry fixed effects:
$$
