...
When S&P 500 corporations discuss artificial intelligence, which suppliers do they name? Commercial names alone answer it -- companies, cloud platforms, accelerators, model families and enterprise AI products, read inside paragraphs that already carry an extracted AI disclosure frame (`configs/ai_vendor_ecosystems.yaml`, Appendix C). Country and demonym words are excluded: "China" or "American" locate a market, not a technology. Ambiguous names need corroboration in the same paragraph, so "copilot" requires Microsoft or GitHub and "arm" an Arm-qualified term; and a firm naming its own products is not counted, since the measure is the reach of external suppliers. Presence is binary per document -- a call naming Azure twenty times counts once for Microsoft and once for the United States -- over a denominator of every document with AI disclosure, so the four ecosystems overlap and their rates are independent indicators rather than shares of a partition. The **AI technology stack** (model developers, cloud infrastructure, semiconductors) and **enterprise software** (applied AI inside business platforms) are reported apart: a blended number would hide which of the two is moving.
...
1. **Naming a supplier is a minority practice, and the supplier is American.** `{python} f"{ve_firms_named_pct:.1f}"`% of the `{python} f"{ve_n_firms:,}"` firms with AI disclosure name any external provider; `{python} f"{ve_us:.1f}"`% name a U.S. technology stack, against `{python} f"{ve_cn:.1f}"`% for China, `{python} f"{ve_ot:.1f}"`% for the rest of the world and `{python} f"{ve_eu:.1f}"`% for Europe. The concentration runs through four providers -- Google (`{python} f"{ve_fam['Google']['pct_firms']:.1f}"`% of firms), Microsoft (`{python} f"{ve_fam['Microsoft']['pct_firms']:.1f}"`%), Amazon (`{python} f"{ve_fam['Amazon']['pct_firms']:.1f}"`%) and OpenAI (`{python} f"{ve_fam['OpenAI']['pct_firms']:.1f}"`%) -- with NVIDIA (`{python} f"{ve_fam['NVIDIA']['pct_firms']:.1f}"`%) behind them. No non-U.S. provider reaches a tenth of firms; the highest are DeepSeek (`{python} f"{ve_fam['DeepSeek']['pct_firms']:.1f}"`%) and Samsung (`{python} f"{ve_fam['Samsung']['pct_firms']:.1f}"`%).
2. **Chinese providers appear in one year and then recede.** Chinese stack mentions stay below 2% of documents in every year except `{python} ve_cn_peak_year`, when they reach `{python} f"{ve_cn_peak:.1f}"`%, and fall to `{python} f"{ve_cn_2026:.1f}"`% in the 2026 year-to-date sample. DeepSeek alone accounts for the movement, from no mentions before 2025 to `{python} f"{ve_rate('DeepSeek', 2025):.1f}"`% of documents in 2025 and `{python} f"{ve_rate('DeepSeek', 2026):.1f}"`% afterwards -- an episode in who gets named, not a durable Chinese presence in the disclosed stack.
3. **Supplier attribution travels with substantive disclosure.** `{python} f"{ve_pb['stack_rate']['United States'][0]:.1f}"`% of Vocal Substantive documents name a U.S. stack provider, roughly twice the rate of Governance-Led (`{python} f"{ve_pb['stack_rate']['United States'][1]:.1f}"`%) and Defensive (`{python} f"{ve_pb['stack_rate']['United States'][2]:.1f}"`%) documents, with the gap widening proportionally for every other ecosystem.
