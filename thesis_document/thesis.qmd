...
The empirical investigation focuses on constituents of the S&P 500 index fixed as of January 1, 2021 (`{python} uni_frame_total` unique issuers) tracked across six annual reporting cycles (2021–2026). Establishing an ex-ante cohort prior to the commercial diffusion of generative AI provides an uncompromised baseline of large-cap U.S. enterprises and prevents look-ahead selection bias: defining the universe using later rosters would systematically oversample technology winners that entered the index due to post-2022 market capitalization gains. Furthermore, to guard against survivorship bias, the `{python} uni_frame_delisted` firms subsequently acquired or delisted remain in the panel through their exit dates. In total, `{python} uni_n_firms` firms have processed narrative records, `{python} uni_n_activity_firms` disclose concrete operational AI activities, and `{python} bal_n` firms form a balanced panel observed continuously across all six years.
...
The dispersion across divisions and 2-digit groups in @fig-sic-distribution shows that corporate AI adoption is evaluated across diverse operational cost structures and capital intensities rather than an isolated technology cohort. Because technology, financial services, and healthcare account for over half the sample while 45 smaller SIC groups span the remainder, empirical specifications can separate economy-wide diffusion pressures from industry-specific workflow integration through sectoral controls (detailed in Appendix A.1).

The empirical analysis combines five primary data sources across textual, financial, and technological domains: statutory SEC filings (Forms 10-K, 10-Q, DEF 14A, 8-K, 20-F) retrieved via EDGAR; oral earnings-call transcripts covering management presentations and analyst Q&A; daily market price and return series; standardized SEC XBRL accounting fundamentals; and OECD-classified AI patent records from Google Patents.
...
Statutory filings provide comprehensive coverage of all mandated reports filed during index membership. For voluntary earnings calls, three transcript providers are unioned (Hugging Face, Equibles, and StockAnalysis) with rigorous ticker, host company, and event-type verification; quarters without transcript coverage (`{python} cov_call_gap_pct` of firm-quarters) are treated as unobserved rather than silent. Filing records extend through `{python} cov_last_date`; 2026 observations are year-to-date (covering `{python} cov_10k_2026` firms with 10-Ks, `{python} cov_10q_2026` firms with 10-Qs, and `{python} cov_calls_2026` firms with calls), with trend comparisons scaled via same-calendar-window seasonal adjustment (Appendix A.1). @tbl-documents-by-form-year summarizes the resulting document corpus by statutory and voluntary form.
...
To separate corporate rhetoric from operational initiatives, the empirical framework distinguishes between corporate **claims** and **concrete activities**:

- **From text to claims:** A zero-shot language model identifies statements *about* AI across seven communicative dimensions (subject, technology type, temporal stance, functional concepts, specificity signals, promotional tone, and sentence anchors). Sentence-level anchors enable analysts to trace any classification directly back to source text.
- **From claims to concrete activities:** When a claim describes operational action by the firm itself, a second pass extracts an action triple (verb, technological object, business function), deployment lifecycle stage, technology provenance, and evidentiary backing. This inventory allows comparing initiatives by maturity and evidence rather than comparing vocabulary.

Extracted claims and activities aggregate into six core analytical constructs (@tbl-six-constructs):
...
Together, $D$ and $P$ describe corporate communication; $A$, $G$, and $S$ capture disclosed activity and documentary evidence; and $W$ prioritizes discrepancies for review. Measures aggregate to firm-years via median document rates (Appendix A.4).

*Measurement validation.* Blind manual audits on held-out samples demonstrate 91.2% precision for the relevance gate and 89.1%–95.8% field-level accuracy across extracted activity dimensions, alongside external alignment with OECD AI patent portfolios (Appendix B).

With the measurement architecture established, the remainder of this chapter analyzes the market-wide diffusion of AI disclosures and maps the emergent communication postures across the S&P 500.
...
Seven frame-level dimensions describe Communication Posture ($P$). Disclosure intensity ($D$) captures narrative volume, completing the eight-dimensional strategy space. The archetypes later in this chapter are fitted on the seven posture dimensions alone: how much a firm discloses is a separate question from how it discloses, and mixing the two lets volume dominate the result. @tbl-strategy-dimensions defines each dimension and its operational meaning, while @fig-strategy-dimensions-dist shows their distribution across the S&P 500.
...
Evaluating governance, specificity, and operational maturity separately thus prevents treating narrative intensity as an undifferentiated signal of technical capability.
...
This thesis computes the posture dimensions across all `{python} arch_n_universe` firms. Firms with fewer than five AI frames across 2021–2025 SEC filings receive the rule-based label **No AI** ($n =$ `{python} arch_n_noai`). Across the panel, `{python} f"{100 * (arch_n_universe - arch_n_noai) / arch_n_universe:.1f}"`% of firms disclose enough AI commentary to carry an active posture.

For the active firms ($n =$ `{python} arch_n_fit`), disclosure strategies blend distinct communication registers rather than falling into rigid discrete bins. This thesis applies **Archetypal Analysis** [@cutler1994] to the seven posture dimensions. The method picks $k$ extreme profiles from the edge of the data and writes every firm as a blend of them, so a firm is not assigned to one style but given a share of each -- 60% of one and 40% of another, summing to one. Those shares are what the rest of the chapter uses (formal specification in Appendix C).

### Archetypal profiles and the disclosure simplex

With $k = 3$, every disclosing firm's three shares sum to one, so each firm can be drawn as a point inside a triangle whose corners are the three extreme profiles: a corner means a firm is entirely that style, the middle means it is an even blend. @fig-archetypal-ternary-simplex maps all `{python} arch_n_fit` disclosing index constituents onto this continuous disclosure simplex, displaying consolidated industry centroids alongside boundary and central exemplars:
...
Locating a firm relative to peers thus indicates which filings and disclosures warrant closer review; archetypal position reflects communication strategy rather than underlying technological capability.
...
Firms that answer to the same regulator end up writing about AI in the same way. This reflects institutional isomorphism [@dimaggio1983; @suchman1995]: shared institutional pressures establish sector-level disclosure conventions. For both corporate benchmarking and investment screening, assessing a firm against sector peers is therefore essential before interpreting any communication posture as exceptional.
