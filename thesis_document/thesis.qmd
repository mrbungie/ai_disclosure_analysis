...
When S&P 500 corporations discuss artificial intelligence, which suppliers do they name? The question is answered with commercial names alone -- companies, cloud platforms, accelerators, model families and enterprise AI products -- read inside paragraphs that already carry an extracted AI disclosure frame. A document names an ecosystem when at least one of its AI paragraphs mentions a term belonging to a provider of that ecosystem (`configs/ai_vendor_ecosystems.yaml`; the full lexicon is in Appendix C). Presence is binary per document: a call naming Azure twenty times counts once for Microsoft and once for the United States. The denominator is every document with AI disclosure, the four ecosystems overlap, and the rates are therefore independent indicators rather than shares of a partition.

Three construction rules matter for reading the numbers. Country and demonym words are absent from the lexicon: "China," "European" or "American" locate a market or a regulator, not a technology, and counting them would let a tariff sentence stand in for a supply relationship. Ambiguous names are admitted only with corroboration in the same paragraph -- "copilot" needs Microsoft or GitHub, "arm" needs an Arm-qualified term, "claude" needs Anthropic or a model name -- so ordinary English does not become a vendor. And a firm naming its own products is not counted: the measure is the reach of external ecosystems, not the reach of a firm's own brand.

Two coverages are kept apart rather than pooled. The **AI technology stack** is model developers, cloud infrastructure and semiconductors; **enterprise software** is applied AI inside business platforms. The two answer different questions -- where the capability comes from, and which business software carries it -- and a single blended number would hide which is moving.
...
#| fig-cap: "[Named technology ecosystems in AI disclosure across S&P 500 firms (2021–2025, with 2026 year-to-date). Panel A tracks the share of AI-disclosing documents naming each ecosystem's AI technology stack; Panel B breaks the same rate down by disclosure archetype. Rates are independent presence indicators over a common denominator and do not sum to 100.]{custom-style=\"CaptionText\"}"
#| echo: false
#| fig-width: 9.6
#| fig-height: 3.8
import numpy as np
...
colors = {"United States": "#1D4ED8", "Europe": "#10B981", "China": "#DC2626", "Other": "#64748B"}
...
Four findings stand out (@fig-vendor-ecosystems, @tbl-vendor-providers).

1. **Named suppliers are a minority practice, and when they appear they are American.** `{python} f"{ve_firms_named_pct:.1f}"`% of the `{python} f"{ve_n_firms:,}"` firms with AI disclosure name at least one external provider anywhere in the sample. Within that disclosure, `{python} f"{ve_us:.1f}"`% of firms name a U.S. AI technology stack, against `{python} f"{ve_cn:.1f}"`% for China, `{python} f"{ve_ot:.1f}"`% for the rest of the world and `{python} f"{ve_eu:.1f}"`% for Europe. Naming a supplier is the exception; naming an American one is what the exception looks like.
2. **The concentration runs through four providers.** Google (`{python} f"{ve_fam['Google']['pct_firms']:.1f}"`% of firms), Microsoft (`{python} f"{ve_fam['Microsoft']['pct_firms']:.1f}"`%), Amazon (`{python} f"{ve_fam['Amazon']['pct_firms']:.1f}"`%) and OpenAI (`{python} f"{ve_fam['OpenAI']['pct_firms']:.1f}"`%) are named by between a quarter and a third of disclosing firms each, with NVIDIA (`{python} f"{ve_fam['NVIDIA']['pct_firms']:.1f}"`%) behind them. No non-U.S. provider reaches a tenth: the highest are DeepSeek (`{python} f"{ve_fam['DeepSeek']['pct_firms']:.1f}"`%) and Samsung (`{python} f"{ve_fam['Samsung']['pct_firms']:.1f}"`%).
3. **Chinese providers appear in one year and then recede.** Chinese stack mentions sit below 2% of documents in every year except `{python} ve_cn_peak_year`, when they reach `{python} f"{ve_cn_peak:.1f}"`%, and fall back to `{python} f"{ve_cn_2026:.1f}"`% in the 2026 year-to-date sample. DeepSeek alone accounts for that movement, going from no mentions before 2025 to `{python} f"{ve_rate('DeepSeek', 2025):.1f}"`% of documents in 2025 and `{python} f"{ve_rate('DeepSeek', 2026):.1f}"`% afterwards. The episode is visible in who gets named; it does not establish a durable Chinese presence in the disclosed stack.
4. **Vocal Substantives name suppliers; the other two archetypes mostly do not.** `{python} f"{ve_pb['stack_rate']['United States'][0]:.1f}"`% of Vocal Substantive documents name a U.S. stack provider, roughly twice the rate of Governance-Led (`{python} f"{ve_pb['stack_rate']['United States'][1]:.1f}"`%) and Defensive (`{python} f"{ve_pb['stack_rate']['United States'][2]:.1f}"`%) documents, and the gap widens proportionally for every other ecosystem. Concrete supplier attribution travels with substantive disclosure rather than with disclosure volume.

```{python}
#| label: tbl-vendor-providers
#| tbl-cap: "[Providers named in AI disclosure, by ecosystem and role in the AI stack.]{custom-style=\"CaptionText\"}"
#| echo: false
from tabulate import tabulate
from IPython.display import Markdown
...
rows = [[f["family"], f["ecosystem"], TYPE_LABEL[f["type"]], f"{f['pct_firms']:.1f}%", f"{f['n_documents']:,}"]
        for f in ve["families"][:15]]
Markdown(tabulate(rows, headers=["Provider family", "Ecosystem", "Role", "% of firms naming", "Documents"]))
```

::: {custom-style="FigureNote"}
*Notes:* The fifteen most-named provider families of the `{python} f"{len(ve['families'])}"` present in the corpus. "% of firms naming" is the share of the `{python} f"{ve_n_firms:,}"` firms with AI disclosure whose AI paragraphs name that family at least once; "Documents" counts AI-disclosing documents doing so, out of `{python} f"{ve_n_docs:,}"`. Mentions of a firm's own products are excluded.
:::
...
### AI vendor lexicon {.unnumbered}

The commercial names behind @fig-vendor-ecosystems and @tbl-vendor-providers are listed in `configs/ai_vendor_ecosystems.yaml`, one entry per provider family with its ecosystem, its specific country and its role in the AI stack. @tbl-c3-vendor-lexicon summarizes the lexicon's coverage; the matched term, the family and the paragraph it was found in are kept in full in the `ai_vendor_mentions` table, so every presence flag in the figure can be traced back to the sentence that produced it.

```{python}
#| label: tbl-c3-vendor-lexicon
#| tbl-cap: "[Coverage of the AI vendor lexicon, by ecosystem and role.]{custom-style=\"CaptionText\"}"
#| echo: false
import yaml
from tabulate import tabulate
from IPython.display import Markdown

_lex = yaml.safe_load((L.REPO_ROOT / "configs" / "ai_vendor_ecosystems.yaml").read_text())
_present = {f["family"] for f in ve["families"]}
_rows = []
for eco in ["United States", "China", "Europe", "Other"]:
    fams = [v for v in _lex["vendors"] if v["ecosystem"] == eco]
    roles = sorted({TYPE_LABEL[v["type"]] for v in fams})
    _rows.append([eco, len(fams), sum(len(v["terms"]) for v in fams),
                  sum(1 for v in fams if v["family"] in _present), ", ".join(roles)])
Markdown(tabulate(_rows, headers=["Ecosystem", "Families", "Terms", "Families found", "Roles covered"]))
```

::: {custom-style="FigureNote"}
*Notes:* "Families found" counts the provider families at least one firm names in its AI disclosure; a family nobody mentions scores zero rather than being dropped from the lexicon. Country and demonym terms are not part of the lexicon. `{python} f"{len(_lex['context_required'])}"` terms are ordinary words, personal names or multi-vendor products ("copilot", "gemini", "claude", "foundry", "arm") and are counted only when a corroborating term appears in the same paragraph. Product names are kept in their historical form, so a 2023 paragraph saying "Bard" and a 2025 paragraph saying "Gemini" both resolve to Google.
:::
