...
@tbl-e-10k-refit recomputes the archetypal weights ($w_{\text{Vocal}}, w_{\text{Gov}}, w_{\text{Def}}$) on Form 10-K frames alone and correlates them, per firm, with the full-corpus weights reported in Chapter 2 (Section 2.5). The last two rows report the standardized-unit (SD) location of the Governance-Led and Defensive vertices on the risk-orientation dimension, full corpus vs. 10-K-only, to show where the two disagree.
