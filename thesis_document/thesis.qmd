...
Market outcomes line up with how much a firm has historically talked about AI, and not with the substance behind a given quarter's claims. Firms that talk more about AI already carry more risk before any given call, so nothing here says the call moves the market; what the associations do say is that verifiable substance adds nothing measurable once narrative volume is in the model. An allocator cannot read grounding off a price. Persistent disclosure volume is also associated with lower downside crash risk, while a governance-oriented disclosure posture shows the same direction nominally but not once the test family is corrected for multiple comparisons. Rather than labeling any firm as fraudulent or technologically capable, the framework gives managers, boards and investors a disciplined, three-step way to move beyond AI mention counts: identify a firm's disclosure posture, test whether that posture is backed by grounded execution, and check whether the same claims hold from the earnings call to the statutory filing.
...
Research on corporate narrative disclosure has established that how firms write carries information beyond what they report in numbers [@li2008; @loughran2011; @gentzkow2019; @hoberg2016; @hassan2019], and recent work documents a surge in corporate AI discussion since the generative-AI shock [@anantharaman2026]. Studies have characterized that surge by separating actionable from speculative text [@basnet2025], by measuring adoption outside disclosure through patents and job postings [@song2026; @babina2024; @eisfeldt2023], and by validating disclosure against employment data [@barrios2025]. Alongside it, scrutiny has grown over what @elhajjar2025 call "AI washing" -- overstating AI capability for market advantage [@gensler2023; @sec2024pressrelease] -- which extends established findings on greenwashing and symbolic impression management [@delmas2011; @lyon2015; @marquis2016] and on managers who adjust tone independently of fundamentals during technology booms [@huang2014; @cheng2019].

What none of this work does is read the claim itself. External benchmarks tell us whether a firm is adopting AI; they do not tell us whether its own disclosure is internally consistent -- whether the volume of AI language a firm produces is matched by the operational detail it puts on the record. Four mechanisms motivate treating that as the object of measurement. Claims become credible when they carry detail that is costly to fabricate or easy to check [@spence1973; @crawford1982; @dye1985; @rogers2011], so this thesis operationalizes evidence as named systems, deployment stages, quantified metrics and identified partners. Firms under the same regulators and the same peer pressure converge on the same vocabulary whether or not they converge on the same practice [@dimaggio1983; @meyer1977; @suchman1995], so counting mentions measures the pressure to mention. Formal statements can drift from operations [@westphal1994; @crilly2012; @bromley2012; @seele2022], which is what a gap between narrative volume and grounded activity within one firm makes observable. And statutory filings carry a legal exposure that earnings calls do not [@matsumoto2011; @frankel1999], so whether a claim travels from the call into the filing is itself evidence.

That leaves investors and boards with three practical questions this thesis sets out to answer:
...
To address the research questions above, this thesis constructs an empirical pipeline to extract, verify, and evaluate corporate AI disclosures. The pipeline inherits the premise of textual finance that narrative disclosure carries priced information [@li2008; @gentzkow2019], and departs from its dominant instrument. Dictionary methods score a document by counting words from a fixed list [@loughran2011], which makes a routine risk disclaimer and a detailed account of a deployment equivalent evidence as long as both say "artificial intelligence"; measures built on similarity between documents [@hoberg2016] or on the share of a call spent on a topic [@hassan2019] establish that paragraph-level attention can be measured at scale, but still answer how much a text is about something rather than what it claims. This chapter details the study universe, the multi-source data acquisition protocol, the multi-stage extraction funnel, the construction of six analytical constructs, and validation benchmarks.
...
This thesis follows a structured pipeline consistent with standard data-mining methodology [@fayyad1996; @shearer2000]. In practice, this architecture (@fig-pipeline-architecture; @tbl-funnel-summary) converts raw corporate text into structured metrics through two intermediate extraction layers:
...
Firms that answer to the same regulator end up writing about AI in the same way. This is the mechanism @dimaggio1983 describe: shared rules push an industry toward a shared way of talking, whether or not the firms are doing the same things [@suchman1995]. Corporate posture is therefore substantially an industry norm rather than solely an idiosyncratic firm choice.
...
Where AI enters the business barely changes from one archetype to the next (@fig-archetype-activity-mix). Selling -- AI inside the products and services customers buy -- dominates all three groups, with Vocal Substantives leaning on it hardest and Governance-Led Disclosers splitting more evenly between selling and internal operations. Even among the Governance-Led, risk and security work is a small share of what they report doing: governance describes how that group talks, not what it builds. Defensive Disclosers name the same commercial and operational mix as everyone else, only far less of it. What separates the archetypes is how much they say and how loudly, not what enterprise AI is for. Three filings show what that looks like on the page (@tbl-archetype-activity-examples).
...
ECOSYSTEM_COLORS = {"United States": "#1D4ED8", "Europe": "#10B981", "China": "#DC2626", "Other": "#64748B"}
```
...
::: {custom-style="FigureNote"}
*Notes:* The bars are indicators over the same statements, not a partition: the first two overlap, the last two decompose the second, and a statement naming nothing counts in none, so the shares do not sum to 100. An external source is a named foundation model or technology vendor (`{python} f"{ext_vendor_pct:.1f}"`% of statements), a named commercial partner (`{python} f"{partner_pct:.1f}"`%), or the statement declaring the technology itself as externally sourced (`{python} f"{third_party_src:.1f}"`%). Two detectors read the same statement and it counts as naming a source when either finds one. The extraction tags each named entity with a role, catching in-house product names no list can enumerate; the vendor lexicon matches commercial names in the statement's text and resolves them to a provider family, catching suppliers the extraction left untagged. External attribution is `{python} f"{ext_tagged_pct:.1f}"`% on the tagged roles alone and `{python} f"{ext_lexicon_pct:.1f}"`% on the commercial names alone.
:::

Disclosing issuers consistently prioritize proprietary branding over external attribution.
...
fig, ax = plt.subplots()
_RATE = 1e2  # per 1,000 words -> per 100,000 words, so the series reads in units, not zeros
...
ax.annotate("DeepSeek-R1\n20 Jan 2025", xy=(_ev_row["x"], _ev_row["rate_per_1k"]),
            ...
            arrowprops=dict(arrowstyle="->", color=MIB["ink_800"], lw=0.9,
                            shrinkA=0, shrinkB=6))
ax.set_ylabel("Open-source mentions per 100,000 words\n(earnings calls)", fontsize=8.6, fontweight="bold")
...
The gap $W$ measures is a narrower, observable version of a much older construct. Organizational research describes formal statements that drift from what an organization does as decoupling [@meyer1977; @westphal1994; @crilly2012; @bromley2012], and the environmental literature turned the same idea into a measurable claim about firms: @delmas2011 and @lyon2015 define greenwashing as selective disclosure of good environmental performance while withholding the rest, and @marquis2016 show it concentrates where scrutiny is weakest. @elhajjar2025 carry the term into AI as the misrepresentation of AI capability for marketing advantage, and @seele2022 name the algorithmic case machinewashing. None of these is observable without an internal audit. $W$ substitutes a comparison the filings themselves permit: how much a firm says about AI against how much grounded activity it discloses, both ranked against its peers in the same year.
...
::: {custom-style="FigureNote"}
*Notes:* Each firm contributes two points -- its percentile rank of disclosure intensity ($D$) and of disclosed substance ($S$) within the `{python} str(DUMBBELL_YEAR)` disclosing panel -- and the line joining them is $W$. Cases are selected by a rule fixed in advance: among firms in the upper half of disclosure intensity that year, the three largest, the three smallest and the three closest-to-zero values of $W$. Firms in the upper half of $D$ are used so that every case shown is one that talks about AI, which is the setting where the gap is a governance question rather than an artifact of silence.
:::
...
::: {custom-style="FigureNote"}
*Notes:* One firm per pole, taken from @fig-ds-dumbbell: the largest, the closest to zero and the smallest $W$ among firms in the upper half of disclosure intensity. Each excerpt is representative of that firm's AI paragraphs, not the single most extreme sentence in the corpus. Both firms rank high on disclosure intensity; what separates them is whether the text names a system, a stage and a result the extraction can record as an activity.
:::
...
Each cell contains exactly two observations: all earnings calls for the fiscal year are pooled into a single call record, while all statutory SEC filings (10-K and 10-Qs) form the filing benchmark. Metrics are evaluated per thousand words or as proportions of disclosed activities. By clustering standard errors at the firm level, the estimator absorbs time-invariant firm unobservables and macroeconomic shocks to identify the within-firm, within-period venue gap ($\text{Call}_{i,t} - \text{Filing}_{i,t}$). Because oral calls involve interactive analyst Q&A alongside management remarks, this estimate captures the net institutional venue difference rather than a pure causal shift in legal liability. The two venues are not interchangeable settings for the same message: @frankel1999 show that conference calls carry information the accompanying disclosures do not, and @matsumoto2011 that the discussion portion is where managers exercise most of that discretion, while the statutory filing is attorney-reviewed and carries the liability a call does not. That asymmetry is the reason a claim's survival from one venue to the other is informative at all.
...
Promotional claims are accompanied by less evidence in the binding filing than in the call, which raises an immediate valuation question: do capital markets penalize ungrounded disclosure, or do investors respond indiscriminately to narrative volume?
...
- **Boards** get more traction from asking management for quantified outcomes and call-filing consistency than from policing generic AI language. The one observed instance of direct SEC scrutiny is consistent with that same question, asked from outside, narrowing a firm's disclosure gap -- which is an argument for asking it internally first. Persistent AI disclosure volume, not any single archetype, is the pattern most robustly associated with lower downside tail risk in this study; the governance-oriented posture points the same direction but is not a reliable substitute for that broader pattern.
