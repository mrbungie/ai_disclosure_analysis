...
A refit of the archetypes on Form 10-K frames alone reproduces Vocal Substantives and Defensive Disclosers closely ($r =$ `{python} f"{_r_vocal_10k:.3f}"` and `{python} f"{_r_def_10k:.3f}"`) but not Governance-Led ($r =$ `{python} f"{_r_gov_10k:.3f}"`, since board-oversight language lives in DEF 14A proxy statements). A review limited strictly to Form 10-K can therefore overlook substantial governance and oversight evidence that appears primarily in proxy statements (full correlations in Appendix D).
...
While the share of Defensive Disclosers rises more than tenfold (`{python} f"{_share_all.loc[2021, 'Defensive Disclosers']:.1f}"`% $\to$ `{python} f"{_share_all.loc[2026, 'Defensive Disclosers']:.1f}"`% of the index), Vocal Substantives remains roughly stable (`{python} f"{_share_all.loc[2021, 'Vocal Substantives']:.1f}"`% $\to$ `{python} f"{_share_all.loc[2026, 'Vocal Substantives']:.1f}"`%), showing that the space vacated by No AI (`{python} f"{_share_all.loc[2021, 'No AI']:.1f}"`% $\to$ `{python} f"{_share_all.loc[2026, 'No AI']:.1f}"`%) was absorbed overwhelmingly by defensive compliance rather than assertive commercial claims. Panels B-D isolate the entry channel behind that asymmetry: inflow from No AI into Vocal Substantives never exceeds `{python} f"{max(_trans_pct[p].loc['No AI', 'Vocal Substantives'] for p in _trans_pct):.1f}"`% in any year, while inflow into Defensive climbs from `{python} f"{_trans_pct[(2021, 2022)].loc['No AI', 'Defensive Disclosers']:.1f}"`% to `{python} f"{_trans_pct[(2025, 2026)].loc['No AI', 'Defensive Disclosers']:.1f}"`% and into Governance-Led from `{python} f"{_trans_pct[(2021, 2022)].loc['No AI', 'Governance-Led Disclosers']:.1f}"`% to `{python} f"{_trans_pct[(2025, 2026)].loc['No AI', 'Governance-Led Disclosers']:.1f}"`%, confirming that new entrants choose between Governance-Led and Defensive, not Vocal Substantives. Firm-level persistence falls from `{python} f"{_persist[(2021, 2022)]:.1f}"`% (2021--2022) to a low of `{python} f"{_persist[(2023, 2024)]:.1f}"`% at the 2023-2024 break, before recovering to `{python} f"{_persist[(2024, 2025)]:.1f}"`%-`{python} f"{_persist[(2025, 2026)]:.1f}"`% (full transition matrices in supplementary analytics). For monitoring, an observed posture shift flags the need to review newly filed disclosures; on its own, it does not prove an underlying operational transformation.
...
Only $k = 3$ clears the 0.60 Jaccard threshold across both bootstrap regimes; at $k = 4$, partition stability breaks down globally as the candidate fourth cluster captures too few, highly unstable firms to constitute a discrete archetype. The 3-archetype partition also demonstrates out-of-sample stability when projecting across unseen cohorts (Appendix C details complete diagnostics, $k = 2$ comparisons, and K-means cross-checks).
...
Practically, this taxonomy gives corporate leaders an objective benchmark to compare their disclosure emphasis against industry peers, and provides investors with an initial screen to decide where deeper due diligence is required. Evaluating whether these postures reflect genuine execution, however, requires examining the concrete operational activities firms report -- the focus of Chapter 3.
