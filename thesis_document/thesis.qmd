...
**Cumulative disclosure intensity ($D$) is the most consistent predictor of post-call market outcomes.** Trailing disclosure intensity associates with larger market capitalization (MCap: $\beta =$ `{python} b_of("log_market_cap_post", "intensity_expanding")`, $p$ `{python} p_of("log_market_cap_post", "intensity_expanding")`), higher revenue growth ($\beta =$ `{python} b_of("next_revenue_yoy_post", "intensity_expanding")`, $p$ `{python} p_of("next_revenue_yoy_post", "intensity_expanding")`), higher beta ($\beta =$ `{python} b_of("beta_post_63", "intensity_expanding")`, $p$ `{python} p_of("beta_post_63", "intensity_expanding")`), and reduced downside asymmetry (DUVOL: $\beta =$ `{python} b_of("duvol_post", "intensity_expanding")`, $p$ `{python} p_of("duvol_post", "intensity_expanding")`; NCSKEW: $\beta =$ `{python} b_of("ncskew_post", "intensity_expanding")`, $p$ `{python} p_of("ncskew_post", "intensity_expanding")`). MCap, DUVOL, and NCSKEW survive 5% Benjamini-Hochberg false discovery rate (BH-FDR) control across all `{python} str(mkt_m)` tests (threshold `{python} mkt_crit1`), consistent with thematic market exposure [@eisfeldt2023] among established, growing issuers whose returns crash less frequently.

**Decoupling matters as an enduring habit, not a single-call episode.** An individual call's decoupling ($W_{\text{call}}$) provides zero incremental explanatory power across all eight outcomes ($\beta \in [$ `{python} f"{_bat[_bat.variable == 'w_call']['beta_std'].min():+.3f}"`, `{python} f"{_bat[_bat.variable == 'w_call']['beta_std'].max():+.3f}"` $]$). Conversely, accumulated historical decoupling ($W_{\text{exp}}$) reliably predicts higher systematic beta ($\beta =$ `{python} b_of("beta_post_63", "w_expanding")`, $p$ `{python} p_of("beta_post_63", "w_expanding")`), surviving full BH-FDR control. Persistent decoupling behaves as an enduring firm characteristic, reflecting cumulative opacity pricing into systematic risk [@jin2006; @fu2021].

**Posture composition affects market pricing rather than accounting margins.** The joint $F$-test for posture composition is significant for beta ($p =$ `{python} f"{_mkt_joint('beta_post_63'):.3f}"`), MCap ($p =$ `{python} f"{_mkt_joint('log_market_cap_post'):.3f}"`), and DUVOL ($p =$ `{python} f"{_mkt_joint('duvol_post'):.3f}"`), but uninformative for margins or spreads. Reallocating weight toward Governance (vs. Defensive) lowers beta ($\beta =$ `{python} b_of("beta_post_63", "w_gov")`, $p$ `{python} p_of("beta_post_63", "w_gov")`), increases MCap ($\beta =$ `{python} b_of("log_market_cap_post", "w_gov")`, $p$ `{python} p_of("log_market_cap_post", "w_gov")`), and cuts downside risk (DUVOL $\beta =$ `{python} b_of("duvol_post", "w_gov")`, $p$ `{python} p_of("duvol_post", "w_gov")`). Governance also separates cleanly from Vocal posture on beta ($\beta_{\text{gov}} - \beta_{\text{voc}} =$ `{python} f"{ctr('beta_post_63', 'Governance vs. Vocal')[0]:+.3f}"`, $p$ `{python} pstr(ctr('beta_post_63', 'Governance vs. Vocal')[1])`).

**Entering AI discourse carries a discrete extensive-margin tail risk.** Trailing AI disclosure entry ($\beta_{\text{has}}$) associates with higher post-call crash risk across both metrics (NCSKEW: $\beta =$ `{python} b_of("ncskew_post", "has_posture")`, $p$ `{python} p_of("ncskew_post", "has_posture")`; DUVOL: $\beta =$ `{python} b_of("duvol_post", "has_posture")`, $p$ `{python} p_of("duvol_post", "has_posture")`), both surviving BH-FDR control. Thus, entering AI disclosure is conditionally associated with elevated downside asymmetry, whereas scaling cumulative volume ($D$) conditional on speaking attenuates crash risk.

**These findings delineate the practical boundaries of AI disclosure analytics.** For corporate directors and investment allocators, historical disclosure records provide a useful diagnostic complement to traditional financial statement analysis, revealing persistent opacity and governance framing without requiring proprietary data. However, these associations are strictly cross-sectional comparisons between firms rather than causal policy prescriptions; altering disclosure language alone will not alter underlying cost of capital or crash vulnerability. Furthermore, because multiple quarterly earnings calls map to a single annual report, the empirical design has inherently higher statistical resolution for market price responses than for slow-moving accounting fundamentals.

# From Measurement to Managerial Application

The central empirical finding of Chapter 5 is an asymmetry: market outcomes are associated with how much a firm discloses ($D$) and, where the outcome is a market price, with whether it frames AI through governance -- while verifiable operational substance ($S$) and the decoupling of any single call show little incremental association. Market prices therefore do not appear, in this setting, to provide a reliable filter between narrative intensity and disclosed execution, so investment allocators and corporate directors cannot rely on market multiples alone to discipline AI disclosure. To bridge this analytical gap, this chapter operationalizes the measurement architecture into a concrete credibility framework for investment due diligence and corporate disclosure governance.

The framework organizes due diligence into three sequential questions, each answered by the same six constructs used throughout this thesis -- narrative volume ($D$), communication posture ($P$), claimed operational activities ($A$), evidentiary grounding ($G$), substance ($S$), and decoupling ($W$): **Step 1, Posture** -- which communication archetype does the firm's register resemble (Chapter 2)? **Step 2, Execution** -- does that posture correspond to grounded operational substance, or to narrative volume alone? **Step 3, Venue** -- does that execution hold consistently between voluntary earnings calls and statutory Form 10-K filings (Chapter 4)?
...
**Microsoft (MSFT):** the highest narrative volume in the sample is matched, not offset, by one of the largest disclosed activity inventories and the strongest operational grounding in the corpus (@tbl-firm-comparisons). Talk and substance move together almost one-for-one, so exceptional disclosure volume here signals depth of execution rather than promotional excess. *The action:* an analyst screening on volume alone would flag this firm and be wrong, so the screen has to be run on the gap rather than on the level; high grounding on the screen does not eliminate the need to verify specific critical claims, but confirms that overall volume is backed by operational detail.

**JPMorgan Chase (JPM) and Goldman Sachs (GS):** both exhibit negative decoupling ($W < 0$), disclosing more grounded activity relative to their peer rank than their narrative volume suggests, and both frame AI through controls rather than commercial claims. *The action:* the diligence question here is not about over-claiming but about visibility. An investor comparing the two against a louder peer should ask what is being built that the disclosure does not describe, and a board at either firm should ask whether under-disclosure is a deliberate competitive choice or an unmanaged one.

**Apple (AAPL):** a governance-oriented posture with moderate narrative presence and a positive gap -- privacy framing carries the AI discussion further than the disclosed activity behind it. *The action:* disclosure review should focus on clarifying the scope, maturity, and evidentiary backing of strategic claims, distinguishing broad governance commitments from concrete system deployments.

**Welltower (WELL):** the clearest instance of cross-venue divergence in the sample -- oral earnings-call claims running well ahead of statutory 10-K disclosure -- and the one case that drew a direct SEC comment letter, examined in Chapter 4 and Appendix E. *The action:* the discrepancy was available to any reader who put the call transcript beside the 10-K before the regulator did. Issuers should reconcile prepared call remarks against the filing language ahead of the call; an investor or a reviewer should read the two documents together, since neither one alone shows the gap.
